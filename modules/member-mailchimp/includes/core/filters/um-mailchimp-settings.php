...
function um_mailchimp_settings( $settings ) {
	...
	$settings['licenses']['fields'][] = array(
		...
		'label'    		=> __( 'MailChimp License Key', 'twodayssss' ),
		'item_name'     => 'MailChimp',
	...
	$settings['extensions']['sections'][$key] = array(
		'title'     => __( 'MailChimp', 'twodayssss' ),
		'fields'    => array(
			array(
				'id'       		=> 'mailchimp_api',
				'type'     		=> 'mailchimp_api_key',
				'label'   		=> __( 'MailChimp API Key','twodayssss' ),
				'tooltip' 	=> __('The MailChimp API Key is required and enables you access and integration with your lists.','twodayssss'),
				'size' => 'medium',
			),
			...
			array(
				...
				'type'     		=> 'checkbox',
				'label'   		=> __( 'Remove subscriber from Mailchimp list when user unsubscribed','twodayssss' ),
				'tooltip' 	=> __('If set option then subscriber will be removed from Mailchimp list','twodayssss'),
			),
			...
			array(
				...
				'type'     		=> 'checkbox',
				'label'		    => __( 'Enable requests log' ,'twodayssss' ),
				'tooltip'		=> __( 'Log all requests to mailchimp server and save to wp-content/uploads/ultimatemember/mailchimp.log.', 'twodayssss'),
			),
...
function um_render_field_type_mailchimp_log() {
	...
	<p>
		...
		<button class="button" id="um_mailchimp_clear_log"><?php _e('Clear log', 'twodayssss') ?></button>
	</p>
...
function um_mailchimp_email_registration_data( $data ) {
	if ( isset( $data['twodayssss'] ) ) {
		 $array_lists = array();
		foreach( $data['twodayssss'] as $list_id => $val ) {
		...
		$data[ __('Mailchimp Subscription','twodayssss') ] = implode(", ", $array_lists );
		unset( $data['twodayssss'] );
	}
	return $data;
}