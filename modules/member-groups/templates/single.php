...
<div class="um um-groups-single">
	...
	<div class='um-group-single-header'>
		...
		<div class='um-group-privacy'>
			<?php echo UM()->Groups()->api()->get_single_privacy( get_the_ID() ).' '.__('Group','twodayssss'); ?>
		</div>
		
		<div class='um-group-members-count'>
			<?php $count = um_groups_get_member_count( get_the_ID(), true );
			echo sprintf( _n( '<span class="count">%s</span> member', '<span class="count">%s</span> members', $count, 'twodayssss' ), number_format_i18n( $count ) ); ?>
		</div>
