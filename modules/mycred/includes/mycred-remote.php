...
if ( ! function_exists( 'mycred_remote_activate_row' ) ) :
	function mycred_remote_activate_row( $mycred_general ) {
		...
		if ( ! get_option( 'permalink_structure' ) ) {
			...
			$disabled_message = __( 'This feature requires WordPress Permalinks to be setup and enabled!', 'twodayssss' );
		}

		elseif ( ! $settings['enabled'] )
			$disabled_message = __( 'Click Update Settings to load the Remote API settings.', 'twodayssss' );

?>
<label class="subheader" for=""><?php _e( 'Allow Remote Access', 'twodayssss' ); ?></label>
...
?>
<h4><span class="dashicons dashicons-cloud <?php if ( empty( $settings['key'] ) ) echo 'static'; else echo 'active'; ?>"></span><?php _e( 'Remote Access', 'twodayssss' ); ?></h4>
<div class="body" style="display:none;">
	<label class="subheader"><?php _e( 'API Key', 'twodayssss' ); ?></label>
	<ol id="myCRED-remote-api-key" class="inline">
		<li>
			<label><?php _e( 'Key', 'twodayssss' ); ?></label>
			<div class="h2"><input type="text" name="mycred_pref_core[remote][key]" id="myCRED-remote-key" value="<?php echo esc_attr( $settings['key'] ); ?>" style="width:90%;" placeholder="<?php _e( '16, 24 or 32 characters', 'twodayssss' ); ?>" /></div>
			<span class="description"><?php _e( 'Required for this feature to work!<br />Minimum 12 characters.', 'twodayssss' ); ?></span>
		</li>
		<li>
			<label><?php _e( 'Key Length', 'twodayssss' ); ?></label>
		...
		<li>
			<label>&nbsp;</label><br />
			<input type="button" id="mycred-generate-api-key" value="<?php _e( 'Generate New Key', 'twodayssss' ); ?>" class="button button-large button-primary" />
		</li>
		<li class="block"><p><strong><?php _e( 'Warning!', 'twodayssss' ); ?></strong> <?php echo $mycred_general->core->template_tags_general( __( 'Keep this key safe! Those you share this key with will be able to remotely deduct / add / transfer %plural%!', 'twodayssss' ) ); ?></p></li>
	</ol>
	<label class="subheader"><?php _e( 'Incoming URI', 'twodayssss' ); ?></label>
	<ol id="myCRED-remote-api-uri">
		<li>
			...
			<span class="description"><?php _e( 'The incoming call address. Remote calls made to any other URL will be ignored.', 'twodayssss' ); ?></span>
		</li>
	</ol>
	<label class="subheader" for=""><?php _e( 'Debug Mode', 'twodayssss' ); ?></label>
	<ol id="myCRED-remote-api-debug-mode">
		<li>
			<input type="checkbox" name="mycred_pref_core[remote][debug]" id="myCRED-remote-debug-mode"<?php if ( $settings['debug'] ) echo ' checked="checked"'; ?> value="1" /> <span class="description"><?php _e( 'Remember to disable when not used to prevent mischievous calls from learning about your setup!', 'twodayssss' ); ?></span></li>
	</ol>
