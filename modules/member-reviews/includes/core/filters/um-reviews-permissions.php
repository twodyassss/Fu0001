...
function um_reviews_user_menu( $items, $user_id ) {
	...
	if ( UM()->roles()->um_current_user_can( 'edit', $user_id ) ) {
		...
		if ( ! UM()->Reviews_API()->api()->is_blocked( $user_id ) ) {
			$items['reviews_block'] = '<a href="' . $block_reviews_link . '" class="real_url">' . __( 'Block reviews', 'twodayssss' ) . '</a>';
		} else {
			$items['reviews_block'] = '<a href="' . $unblock_reviews_link . '" class="real_url">' . __( 'Unblock reviews', 'twodayssss' ) . '</a>';
		}
	}
