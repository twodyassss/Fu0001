...
<span class="um-badges">
	<?php foreach ( $all_badges as $badge_id ) {
		...
		<span class="um-badge-leaderboard title">
			<strong><?php _e( $badge->title, 'twodayssss') ?></strong>
		</span>
		...
		foreach ( $badge->levels as $level_key => $level ) {
			...
			if ( $level['attachment_id'] > 0 ) {
				...
				if ( ! empty( $level_title ) ) {
					...
					$badge_title = sprintf( __( "%s - Level %s", 'twodayssss' ), $badge->title, $level_key__count );
				}
			} else {
				$image_identification = $level['attachment_id'];
			}
			...
			if ( ! empty( $base_requirements ) ) {
				foreach ( $base_requirements as $requirement ) {
					if ( ( $badge_level_reached !== false && $level_key <= $badge_level_reached ) ) { // Unlocked
						$badge_title .= sprintf( _x( '&#013;&#10;%s %s for "%s"', '"Gained/Lost" "x points" for "reference"', 'twodayssss' ), ( ( $requirement['amount'] < 0 ) ? __( 'Lost', 'twodayssss' ) : __( 'Gained', 'twodayssss' ) ), $mycred->format_creds( $requirement['amount'] ), $requirement['ref'] );
					} else { // Locked
						$badge_title .= sprintf( _x( '&#013;&#10;%s %s for "%s"', '"Gain/Lost" "x points" for "reference"', 'twodayssss' ), ( ( $requirement['amount'] < 0 ) ? __( 'Lost', 'twodayssss' ) : __( 'Gain', 'twodayssss' ) ), $mycred->format_creds( $requirement['amount'] ), $requirement['ref'] );
					}
				}
			}
