	...
	function call() {
		...
		if ( !$apikey )
			return new \WP_Error( 'um-mailchimp-empty-api-key',
				sprintf(__('<a href="%s"><strong>Please enter your valid API key</strong></a> in settings.','twodayssss'), admin_url('admin.php?page=um_options&tab=extensions&section=mailchimp' ) ) );
	...
	function bulk_subscribe_process( $list_id, $users, $status = 'subscribed', $action_key = '', $merge_vars = false ) {
		if( count( $users ) ) {
			...
			if( is_numeric( $list_id ) && !( $list = UM()->Mailchimp_API()->api()->fetch_list( $list_id ) ) ) {
				return new \WP_Error('um_mailchimp_wrong_list', __('Wrong list', 'twodayssss') );
			}
