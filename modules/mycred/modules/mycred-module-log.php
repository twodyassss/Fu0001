	...
	class myCRED_Log_Module extends myCRED_Module {
		...
		public function __construct( $type = MYCRED_DEFAULT_TYPE_KEY ) {
			...
			parent::__construct( 'myCRED_Log_Module', array(
				'module_name' => 'log',
				'labels'      => array(
					'menu'        => __( 'Log', 'twodayssss' ),
					'page_title'  => __( 'Log', 'twodayssss' )
				),
		...
		public function action_delete_log_entry() {
			...
			// Respond
			wp_send_json_success( __( 'Row Deleted', 'twodayssss' ) );
		...
		public function action_update_log_entry() {
			...
			if ( $request['ref'] == '' || ! array_key_exists( $request['ref'], $all_references ) )
				wp_send_json_error( array( 'message' => esc_attr__( 'Invalid or empty reference', 'twodayssss' ) ) );

			// Check entry
			$request['entry'] = wp_kses_post( $request['entry'] );
			if ( $request['entry'] == '' && ! $this->core->user_is_point_admin() )
				wp_send_json_error( array( 'message' => esc_attr__( 'Log Entry cannot be empty', 'twodayssss' ) ) );

			// Check amount
			$amount           = $this->core->number( $request['creds'] );
			if ( $amount === $this->core->zero() )
				wp_send_json_error( array( 'message' => esc_attr__( 'Amount can not be zero', 'twodayssss' ) ) );
			...
			if ( ! isset( $log_entry->ref ) )
				wp_send_json_error( array( 'message' => esc_attr__( 'Log entry not found', 'twodayssss' ) ) );
			...
			if ( ! $this->core->update_log_entry( $entry_id, array( 'ref' => $request['ref'], 'creds' => $amount, 'entry' => $request['entry'] ), array( '%s', $format, '%s' ) ) )
				wp_send_json_error( array( 'message' => esc_attr__( 'Could not save the new log entry', 'twodayssss' ) ) );
			...
			wp_send_json_success( array(
				'message' => esc_attr__( 'Log entry successfully updated', 'twodayssss' ),
		...
		public function my_history_menu() {
			...
			$page = add_users_page(
				$this->core->plural() . ' ' . __( 'History', 'twodayssss' ),
				$this->core->plural() . ' ' . __( 'History', 'twodayssss' ),
				'read',
		...
		public function admin_notices() {
			...
			if ( isset( $_GET['deleted'] ) && isset( $_GET['ctype'] ) && $_GET['ctype'] == $this->mycred_type )
				echo '<div id="message" class="updated notice is-dismissible"><p>' . sprintf( _n( '1 Entry Deleted', '%d Entries Deleted', absint( $_GET['deleted'] ), 'twodayssss' ), absint( $_GET['deleted'] ) ) . '</p><button type="button" class="notice-dismiss"></button></div>';
		...
		public function settings_header() {
			...
			wp_localize_script(
				...
				array(
					...
					'title'      => esc_attr__( 'Edit Log Entry', 'twodayssss' ),
					'close'      => esc_attr__( 'Close', 'twodayssss' ),
					'working'    => esc_attr__( 'Processing...', 'twodayssss' ),
					'messages'   => array(
						'delete'     => esc_attr__( 'Are you sure you want to delete this log entry? This can not be undone!', 'twodayssss' ),
						'update'     => esc_attr__( 'The log entry was successfully updated.', 'twodayssss' ),
						'error'      => esc_attr__( 'The selected log entry could not be deleted.', 'twodayssss' ),
					),
					'tokens'     => array(
		...
		public function page_title( $title = 'Log' ) {
			...
			if ( isset( $_GET['s'] ) && ! empty( $_GET['s'] ) )
				$search_for = ' <span class="subtitle">' . __( 'Search results for', 'twodayssss' ) . ' "' . $_GET['s'] . '"</span>';

			elseif ( isset( $_GET['time'] ) && $_GET['time'] != '' ) {
				$time       = urldecode( $_GET['time'] );
				$check      = explode( ',', $time );
				$search_for = ' <span class="subtitle">' . sprintf( _x( 'Log entries from %s', 'e.g. Log entries from April 12th 2016', 'twodayssss' ), date( 'F jS Y', $check[0] ) ) . '</span>';
			}
...
<div class="wrap" id="myCRED-wrap">
	<h1><?php _e( 'Log', 'twodayssss' ); if ( MYCRED_DEFAULT_LABEL === 'twodayssss' ) : ?> <a href="" class="page-title-action" target="_blank"><?php _e( 'Documentation', 'twodayssss' ); ?></a><?php endif; ?></h1>
<?php
			...
			if ( ! in_array( 'mcrypt', $extensions ) && ! defined( 'MYCRED_DISABLE_PROTECTION' ) )
				echo '<div id="message" class="error below-h2"><p>' . __( 'Warning. The required Mcrypt PHP Library is not installed on this server! Certain hooks and shortcodes will not work correctly!', 'twodayssss' ) . '</p></div>';
...
?>
<div class="wrap" id="myCRED-wrap">
	<h1><?php $this->page_title( sprintf( __( 'My %s History', 'twodayssss' ),  $this->core->plural() ) ); ?></h1>
		...
		<form class="form" method="post" action="" id="mycred-editor-form">
			...
			<div class="row">
				<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
					<label><?php _e( 'User', 'twodayssss' ); ?></label>
					<div id="mycred-user-to-show"></div>
				</div>
				<div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
					<label><?php _e( 'Date', 'twodayssss' ); ?></label>
			...
			<div class="row">
				<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12" id="mycred-old-entry-to-show-wrapper">
					<label><?php _e( 'Original Entry', 'twodayssss' ); ?></label>
			...
			<div class="row last">
				<div class="col-lg-2 col-md-3 col-sm-12 col-xs-12 text-center">
					<a href="javascript:void(0);" class="button button-primary button-large mycred-delete-row" id="mycred-delete-entry-in-editor" data-id=""><?php _e( 'Delete Entry', 'twodayssss' ); ?></a>
				</div>
				...
				<div class="col-lg-3 col-md-2 col-sm-11 col-xs-12 text-right">
					<input type="submit" id="mycred-editor-submit" class="button button-secondary button-large" value="<?php _e( 'Update Entry', 'twodayssss' ); ?>" />
				</div>
			</div>
		</form>
	</div>
