...
?>
			...
			<div class="um-groups-commentl" id="commentid-<?php echo $comment->comment_ID; ?>">
				...
				<div class="um-groups-comment-hidden hidden-<?php echo $user_hidden; ?>"><?php _e('Comment hidden. <a href="#" class="um-link">Show this comment</a>','twodayssss'); ?></div>
				...
				<div class="um-groups-comment-info hidden-<?php echo $user_hidden; ?>">
					...
					<div class="um-groups-comment-meta">
						...
						<span><a href="#" class="um-link um-groups-comment-like active" data-like_text="<?php _e('Like','twodayssss'); ?>" data-unlike_text="<?php _e('Unlike','twodayssss'); ?>"><?php _e('Unlike','twodayssss'); ?></a></span>
						<?php } else { ?>
						...
						<?php if ( UM()->Groups()->discussion()->can_comment() ) { ?><span><a href="#" class="um-link um-groups-comment-reply" data-commentid="<?php echo $comment->comment_ID; ?>"><?php _e('Reply','twodayssss'); ?></a></span><?php } ?>
		...
		echo '<div class="um-groups-comment-child">';
			...
			if ( count( $child_all ) > count( $child ) ) {
				...
				if ( $calc > 1 ) {
					$text = sprintf(__('load %s more replies','twodayssss'), $calc );
				} else if ( $calc == 1 ) {
					$text = sprintf(__('load %s more reply','twodayssss'), $calc );
				}
				echo '<a href="#" class="um-groups-ccommentload" data-load_replies="'. __('load more replies','twodayssss').'" data-load_comments="'.__('load more comments','twodayssss') . '" data-loaded="'. count( $child ) . '"><i class="um-icon-forward"></i><span>' . $text . '</span></a>';
