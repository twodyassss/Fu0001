					...
					<div class="um-activity-dialog um-activity-tool-dialog">
						...
						if ( current_user_can('edit_users') || $author_id == get_current_user_id() ) { ?>
							<a href="javascript:void(0);" class="um-activity-trash"
							   data-msg="<?php esc_attr_e('Are you sure you want to delete this post?','twodayssss'); ?>">
						...
						if ( $author_id != get_current_user_id() ) { ?>
							...
							<a href="#" class="um-activity-report <?php if ( UM()->Activity_API()->api()->reported( $post->ID ) ) echo 'flagged'; ?>"
							   data-report="<?php esc_attr_e('Report','twodayssss'); ?>"
							   data-cancel_report="<?php esc_attr_e('Cancel report','twodayssss'); ?>">
								<?php echo ( UM()->Activity_API()->api()->reported( $post->ID, get_current_user_id() ) ) ? __('Cancel report','twodayssss') : __('Report','twodayssss'); ?>
							</a>
						<?php } ?>
					...
					<div class="um-activity-left">
						<div class="um-activity-disp-likes">
							<a href="#" class="um-activity-show-likes um-link" data-post_id="<?php echo $post->ID; ?>">
								...
								<span class="um-activity-disp-span"><?php _e('likes','twodayssss'); ?></span>
							</a>
						</div>
						<div class="um-activity-disp-comments">
							<a href="javascript:void(0);" class="um-link">
								...
								<span class="um-activity-disp-span"><?php _e('comments','twodayssss'); ?></span>
							</a>
						</div>
					</div>
			...
			<?php if ( is_user_logged_in() ) { ?>
				...
				<div class="um-activity-left um-activity-actions">
					<?php if ( UM()->Activity_API()->api()->user_liked( $post->ID ) ) { ?>
						<div class="um-activity-like active" data-like_text="<?php _e('Like','twodayssss'); ?>" data-unlike_text="<?php _e('Unlike','twodayssss'); ?>"><a href="#"><i class="um-faicon-thumbs-up um-active-color"></i><span class=""><?php _e('Unlike','twodayssss'); ?></span></a></div>
					<?php } else { ?>
