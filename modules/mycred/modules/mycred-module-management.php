	...
	class myCRED_Management_Module extends myCRED_Module {
		...
		public function ajax_editor_balance_update() {
			...
			$result          = array(
				...
				'label'         => esc_attr__( 'Update Balance', 'twodayssss' ),
				'results'       => '',
				'user_id'       => $user_id,
				'amount'        => $amount,
				'reference'     => $reference,
			...
			if ( $mycred->exclude_user( $user_id ) ) {
				...
				$result['results'] = __( 'User is excluded', 'twodayssss' );
			...
			if ( $mycred->user_is_point_editor() && ! $mycred->user_is_point_admin() && strlen( $entry ) == 0 ) {
				...
				$result['results'] = __( 'Log Entry can not be empty', 'twodayssss' );
				wp_send_json_error( $result );
			...
			if ( $amount == '' || $mycred->number( $amount ) == $mycred->zero() ) {
				...
				$result['results'] = __( 'Amount can not be zero', 'twodayssss' );
				wp_send_json_error( $result );
			...
			if ( $success ) {
				...
				$result['results'] = __( 'Balance successfully updated', 'twodayssss' );

			}
			else {

				$result['results'] = __( 'Request declined', 'twodayssss' );
...
<div class="row last">
	<div class="col-xs-12">
		<p><?php _e( 'No recent activity found.', 'twodayssss' ); ?></p>
	</div>
</div>
<?php
...
<div class="row ledger header">
	<div class="col-xs-4"><strong><?php _e( 'Date', 'twodayssss' ); ?></strong></div>
	<div class="col-xs-4"><strong><?php _e( 'Time', 'twodayssss' ); ?></strong></div>
	<div class="col-xs-4"><strong><?php _e( 'Reference', 'twodayssss' ); ?></strong></div>
	<div class="col-xs-12"><strong><?php _e( 'Entry', 'twodayssss' ); ?></strong></div>
</div>
<?php
...
<div class="row ledger">
	<div class="col-xs-12">
		<div style="text-align:center; padding: 12px 0;"><a href="<?php echo esc_url( admin_url( 'admin.php?page=' . $page . '&user=' . $user_id ) ); ?>" style="width: auto !important;" class="button button-large button-secondary"><?php _e( 'View complete history', 'twodayssss' ); ?></a></div>
	</div>
</div>
<?php
			...
			if ( $screen->id == 'users' ) {
				...
				wp_localize_script(
					...
					array(
						...
						'title'       => esc_attr__( 'Edit Users Balance', 'twodayssss' ),
						'close'       => esc_attr__( 'Close', 'twodayssss' ),
		...
		public function custom_user_column_content( $value, $column_name, $user_id ) {
			...
			$balance .= '<div id="mycred-user-' . $user_id . '-balance-' . $column_name . '"><small style="display:block;">' . sprintf( '<strong>%s</strong>: <span>%s</span>', __( 'Total', 'twodayssss' ), $mycred->format_number( $total ) ) . '</small></div>';
			...
			// Row actions
			$row            = array();
			$row['history'] = '<a href="' . esc_url( admin_url( 'admin.php?page=' . $page . '&user=' . $user_id ) ) . '">' . __( 'History', 'twodayssss' ) . '</a>';
			$row['adjust']  = '<a href="javascript:void(0)" class="mycred-open-points-editor" data-userid="' . $user_id . '" data-current="' . $mycred->format_number( $ubalance ) . '" data-total="' . $mycred->format_number( $total ) . '" data-type="' . $column_name . '" data-username="' . $user->display_name . '" data-zero="' . $mycred->zero() . '">' . __( 'Adjust', 'twodayssss' ) . '</a>';
...
<div id="mycred-edit-user-wrapper">
	<table class="form-table mycred-inline-table">
		<tr>
			<th scope="row"><?php _e( 'Balances', 'twodayssss' ); ?></th>
			<td>
				<fieldset id="mycred-badge-list" class="badge-list">
					<legend class="screen-reader-text"><span><?php _e( 'Balance', 'twodayssss' ); ?></span></legend>
<?php
...
?>
					<div class="mycred-wrapper balance-wrapper disabled-option color-option">
						...
						<div class="balance-row">
							<div class="balance-view"><?php _e( 'Excluded', 'twodayssss' ); ?></div>
...
?>
					<div class="mycred-wrapper balance-wrapper color-option selected">
						<?php if ( $data['can_edit'] ) : ?><div class="toggle-mycred-balance-editor"><a href="javascript:void(0);" data-type="<?php echo $point_type; ?>" data-view="<?php _e( 'Edit', 'twodayssss' ); ?>" data-edit="<?php _e( 'Cancel', 'twodayssss' ); ?>"><?php _e( 'Edit', 'twodayssss' ); ?></a></div><?php endif; ?>
						<div><?php echo $data['name']; ?></div>
...
<div id="edit-mycred-balance" style="display: none;">
	<?php if ( $name == 'twodayssss' ) : ?><img id="mycred-token-sitting" class="hidden-sm hidden-xs" src="<?php echo plugins_url( 'assets/images/token-sitting.png', myCRED_THIS ); ?>" alt="Token looking on" /><?php endif; ?>
		...
		<form class="form" method="post" action="" id="mycred-editor-form">
			...
			<div class="row">
				<div class="col-sm-2 col-xs-6">
					<div class="form-group">
						<label><?php _e( 'ID', 'twodayssss' ); ?></label>
						<div id="mycred-userid-to-show">&nbsp;</div>
					</div>
				</div>
				<div class="col-sm-4 col-xs-6">
					<div class="form-group">
						<label><?php _e( 'Username', 'twodayssss' ); ?></label>
						<div id="mycred-username-to-show">&nbsp;</div>
					</div>
				</div>
				<div class="col-sm-3 col-xs-6">
					<div class="form-group">
						<label><?php _e( 'Current Balance', 'twodayssss' ); ?></label>
						<div id="mycred-current-to-show">&nbsp;</div>
					</div>
				</div>
				<div class="col-sm-3 col-xs-6">
					<div class="form-group">
						<label><?php _e( 'Total Balance', 'twodayssss' ); ?></label>
			...
			<div class="row">
				<div class="col-sm-2 col-xs-12">
					<div class="form-group">
						<label><?php _e( 'Amount', 'twodayssss' ); ?></label>
						<input type="text" name="mycred_manage_balance[amount]" id="mycred-editor-amount" size="8" placeholder="0" value="" />
						<span class="description"><?php _e( 'A positive or negative value', 'twodayssss' ); ?>.</span>
					</div>
				</div>
				<div class="col-sm-5 col-xs-12">
					<div class="form-group">
						<label><?php _e( 'Reference', 'twodayssss' ); ?></label>
...
<?php
				...
				echo '<option value="mycred_custom">' . __( 'Log under a custom reference', 'twodayssss' ) . '</option>';

?>
						</select>
					</div>
					<div id="mycred-custom-reference-wrapper" style="display: none;">
						<input type="text" name="mycred_manage_balance[custom]" id="mycred-editor-custom-reference" placeholder="<?php _e( 'lowercase without empty spaces', 'twodayssss' ); ?>" class="regular-text" value="" />
					</div>
				</div>
				<div class="col-sm-5 col-xs-12">
					<div class="form-group">
						<label><?php _e( 'Log Entry', 'twodayssss' ); ?></label>
						<input type="text" name="mycred_manage_balance[entry]" id="mycred-editor-entry" placeholder="<?php _e( 'optional', 'twodayssss' ); ?>" class="regular-text" value="" />
			...
			<div class="row last">
				<div class="col-sm-2 col-xs-3"><input type="submit" id="mycred-editor-submit" class="button button-primary button-large" value="<?php _e( 'Update', 'twodayssss' ); ?>" /></div>
				...
				<div class="col-sm-3 col-xs-4 text-right"><button type="button" class="button button-secondary button-large" id="load-users-mycred-history"><?php _e( 'Recent Activity', 'twodayssss' ); ?></button></div>
			</div>
		</form>
