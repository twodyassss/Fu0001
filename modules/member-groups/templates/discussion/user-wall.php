			...
			<?php if ( is_user_logged_in() && ( ! $pending_approval || $author_id == get_current_user_id() ) ) { ?>
				...
				<div class="um-groups-dialog um-groups-tool-dialog">
					...
					<?php if ( ( current_user_can('edit_users') || $author_id == get_current_user_id() ) || ( UM()->Groups()->discussion()->get_action_type( $post->ID ) == 'status' ) && ! $pending_approval ) { ?>
						<a href="#" class="um-groups-manage" data-cancel_text="<?php _e('Cancel editing','twodayssss'); ?>" data-update_text="<?php _e('Update','twodayssss'); ?>"><?php _e('Edit','twodayssss'); ?></a>
					<?php } ?>

					<?php if ( current_user_can('edit_users') || $author_id == get_current_user_id() ) { ?>
						<a href="#" class="um-groups-trash" data-msg="<?php _e('Are you sure you want to delete this post?','twodayssss'); ?>"><?php _e('Delete','twodayssss'); ?></a>
					<?php } ?>

					<?php if ( $author_id != get_current_user_id() ) { ?>
						<span class="sep"></span>
						<a href="#" class="um-groups-report <?php if ( UM()->Groups()->discussion()->reported( $post->ID ) ) echo 'flagged'; ?>" data-report="<?php _e('Report','twodayssss'); ?>" data-cancel_report="<?php _e('Cancel report','twodayssss'); ?>"><?php echo ( UM()->Groups()->discussion()->reported( $post->ID ) ) ? __('Cancel report','twodayssss') : __('Report','twodayssss'); ?></a>
					<?php } ?>
				...
				<div class="um-groups-left">
					<div class="um-groups-disp-likes">
						<a href="#" class="um-groups-show-likes um-link" data-post_id="<?php echo $post->ID; ?>">
							...
							<span class="um-groups-disp-span"><?php _e('likes','twodayssss'); ?></span>
						</a>
					</div>
					<div class="um-groups-disp-comments">
						<a href="#" class="um-link">
							<span class="um-groups-post-comments"><?php echo $comments; ?></span>
							<span class="um-groups-disp-span"><?php _e('comments','twodayssss'); ?></span>
						</a>
					</div>
				</div>
	...
	?>
		<div class="um-groups-foot status" id="wallcomments-<?php echo $post->ID; ?>">
			...
			<div class="um-groups-left um-groups-actions">
				...
				<div class="um-groups-like active" data-like_text="<?php _e('Like','twodayssss'); ?>" data-unlike_text="<?php _e('Unlike','twodayssss'); ?>"><a href="#"><i class="um-faicon-thumbs-up um-active-color"></i><span class=""><?php _e('Unlike','twodayssss'); ?></span></a></div>
				<?php } else { ?>
				<div class="um-groups-like" data-like_text="<?php _e('Like','twodayssss'); ?>" data-unlike_text="<?php _e('Unlike','twodayssss'); ?>"><a href="#"><i class="um-faicon-thumbs-up"></i><span class=""><?php _e('Like','twodayssss'); ?></span></a></div>
				<?php } ?>
				<?php if ( UM()->Groups()->discussion()->can_comment() ) { ?>
				<div class="um-groups-comment"><a href="#"><i class="um-faicon-comment"></i><span class=""><?php _e('Comment','twodayssss'); ?></span></a></div>
				<?php } ?>
			</div>
