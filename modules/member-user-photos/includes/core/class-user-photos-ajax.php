...
class User_Photos_Ajax {
	...
	function create_um_user_photos_album() {
		...
		if (! wp_verify_nonce( $_POST['_wpnonce'],'um_add_album')){
			...
			$response['messages'][] = __('Invalid nonce','twodayssss');
		...
		if(! isset($_POST['title']) || trim($_POST['title']) == ''){
			$error = true;
			$response['messages'][] = __('Album title is required','twodayssss');
		}
		...
		if(isset($_FILES['album_cover']['tmp_name']) && $_FILES['album_cover']['tmp_name'] !=''){
			...
			if(! in_array($_FILES['album_cover']['type'],$allowed)){
				$error = true;
				$response['messages'][] = $_FILES['album_cover']['type'].' '.__('files are not allowed','twodayssss');
			}
		...
		if(! is_user_logged_in()){
			$error = true;
			$response['messages'][] = __('Invalid request','twodayssss');
		}
	...
	function update_um_user_photos_image() {
		...
		if(! isset($_POST['title']) || trim($_POST['title']) ==''){
			$error = true;
			$response['type'] = 'error';
			$response['messages'][] = __('Title is required','twodayssss');
		}
		...
		*/
		...
		$success_text = __('Update successfull','twodayssss');
		echo json_encode([
			'type' => 'success',
	...
	function download_my_photos() {
		...
		if (! is_user_logged_in()){
			echo __('Invalid request','twodayssss');;
			die;
		}

		if (! class_exists( '\ZipArchive' ) ) {
			echo __('Your download could not be created. It looks like you do not have ZipArchive installed on your server.','twodayssss');
			die;
		}
	...
	public function validate_image_data( $file ) {
		...
		if ( is_wp_error( $image ) ) {
			return __( 'Your image is invalid!', 'twodayssss' );
		}
		...
		if ( isset( $image_info['invalid_image'] ) && $image_info['invalid_image'] == true ) {
			$error = __( 'Your image is invalid or too large!', 'twodayssss' );
		}
