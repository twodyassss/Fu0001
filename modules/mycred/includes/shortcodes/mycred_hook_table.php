...
if ( ! function_exists( 'mycred_render_shortcode_hook_table' ) ) :
	function mycred_render_shortcode_hook_table( $atts ) {
		...
		extract( shortcode_atts( array(
			...
			'comment' => '-comment-',
			'amount'  => '',
			'nothing' => __( 'No instances found for this point type', 'twodayssss' )
		), $atts, MYCRED_SLUG . '_hook_table' ) );

		if ( ! mycred_point_type_exists( $type ) ) return __( 'Point type not found.', 'twodayssss' );
...
<div class="table-responsive">
	<table class="table mycred-hook-table hook-table-<?php echo $id; ?>">
		<thead>
			<tr>
				<th class="column-instance" style="width: 60%;"><?php _e( 'Instance', 'twodayssss' ); ?></th>
				<th class="column-amount" style="width: 20%;"><?php _e( 'Amount', 'twodayssss' ); ?></th>
				<th class="column-limit" style="width: 20%;"><?php _e( 'Limit', 'twodayssss' ); ?></th>
			</tr>
		</thead>
		<tbody>
<?php
				...
				if( $id == "approved" ) {
				    if ( isset( $hooks["hook_prefs"]["comments"]["limits"] ) ) {
				        ...
				        if( (int) $approved_limits["per_post"] > 0 && (int) $approved_limits["per_day"] > 0 ) {
				            ...
				            $limit = sprintf( __( 'Maximum %s times per post and Maximum %s times per day', 'twodayssss' ), $approved_limits["per_post"],  $approved_limits["per_day"] );
				        }
				        elseif( (int) $approved_limits["per_post"] > 0 && (int) $approved_limits["per_day"] < 1 ) {
				        	
				            $limit = sprintf( __( 'Maximum %s times per post', 'twodayssss' ), $approved_limits["per_post"] );
				        }
				        elseif( (int) $approved_limits["per_post"] < 1 && (int) $approved_limits["per_day"] > 0 ) {

				            $limit = sprintf( __( 'Maximum %s times per day', 'twodayssss' ), $approved_limits["per_day"] );
				        }
				        else {
				            $limit = __('No limit', 'twodayssss');
				        }
				    }   
			    }
			    else {
