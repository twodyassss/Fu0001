...
function um_groups_join_button( $group_id = 0 ) {
	...
	if($privacy === 'public_role') {
		...
		$privacy       = ( $array_intersect > 0 ) ? 'public' : 'private';
	}
			...
			switch ( $privacy ) {
				...
				case 'private':
					if( is_user_logged_in() ){
							...
							echo '<a href="'.um_get_core_page('login').'?redirect_to='.get_the_permalink( $group_id ).'" class="um-button um-groups-btn-guest" >'.__('Join Group', 'twodayssss' ).'</a>';
					}
					break;
...
function um_groups_publisher_errors_hook( $arr_posts ) {
	...
	if ( ! wp_verify_nonce( $nonce, 'um-groups-nonce_'.get_current_user_id()  ) ) {
		...
		if( isset( $arr_posts['group_name'] ) && empty( $arr_posts['group_name']  ) ){
			UM()->form()->add_error('group_name', __('You must enter a group name','twodayssss') );
		}

		if( isset( $arr_posts['group_name'] ) && ! empty( $arr_posts['group_name']  ) && strlen( $arr_posts['group_name'] ) < 3 ){
			UM()->form()->add_error('group_name', __('Minimum of 3 characters are allowed.','twodayssss') );
		}

		if( isset( $arr_posts['group_description'] ) && empty( $arr_posts['group_description']  ) ){
			UM()->form()->add_error('group_description', __('You must enter a description','twodayssss') );
		}

		if( isset( $arr_posts['categories'] ) && empty( $arr_posts['categories']  ) ){
			UM()->form()->add_error('categories', __('You must select a category','twodayssss') );
		}

		if( ! isset( $arr_posts['group_tags'] ) || isset( $arr_posts['group_tags'] ) && empty( $arr_posts['group_tags']  ) ){
			UM()->form()->add_error('group_tags', __('You must select a tag','twodayssss') );
		}
	}
}
...
function um_groups_upload_file_errors_hook( $post ) {
	...
	if ( ! wp_verify_nonce( $nonce, 'um-groups-nonce_upload_'.get_current_user_id()  ) ) {
	    ...
	    wp_die( __('Invalid Nonce.','twodayssss') );

	} else {

		if( ! UM()->Groups()->api()->can_manage_group( get_the_ID() ) && ! um_groups_admin_all_access() ){
			wp_die( __('You don\'t have a permission to change something in this group!','twodayssss') );
		}

		if( isset( $arr_file['um_groups_avatar']['error'] ) && $arr_file['um_groups_avatar']['error'] > 0 ){
			UM()->form()->add_error('um_groups_avatar', __('You must select an image file','twodayssss') );
		}
...
function um_groups_delete_group_process_form( $arr_posts ) {
	...
	if ( $has_deleted ) {
		...
		UM()->form()->add_error('um_groups_avatar', __('Something went wrong.','twodayssss') );
	}
}
...
function um_groups_form_notice( $arr_settings ) {
	...
	if ( $updated == 1 ) {
		echo '<p class="um-notice success"><i class="um-icon-ios-close-empty" onclick="jQuery(this).parent().fadeOut();"></i>'.__('Group was updated successfully.','twodayssss').'</p>';
	}
}
