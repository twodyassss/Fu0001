<p class="sub"><?php _e('Connection status','twodayssss'); ?></p>
...
<?php } else {
	if ( isset( $result['account_name'] ) ) { ?>
		<p>
			<?php printf( __( 'Your site is successfully <strong><span class="ok">linked</span></strong> to <strong>%s</strong> MailChimp account.', 'twodayssss' ), $result['account_name'] ); ?>
		</p>
	<?php } ?>

	<p class="sub">
		<?php _e( 'Account status', 'twodayssss' ); ?>
	</p>

	<?php $external_lists = UM()->Mailchimp_API()->api()->get_lists(); ?>
	<p><?php printf( _n('%d subscriber',' %d subscribers',$result['total_subscribers'],'twodayssss' ), $result['total_subscribers'] ); ?>
	<p><?php printf( _n('%d Mailchimp list','%d Mailchimp lists',count( $external_lists ), 'twodayssss'), count( $external_lists ) ); ?></p>

	<?php $internal_lists = UM()->Mailchimp_API()->api()->get_lists( false ); ?>
	<p><?php printf( _n('%d UM Mailchimp list','%d UM Mailchimp lists',count( $internal_lists ), 'twodayssss'), count( $internal_lists ) ); ?></p>
	...
	<script type="text/html" id="tmpl-um-mailchimp-sync-metabox">
		<div class="um_mailchimp_metabox">
			<select name="um_mailchimp_sync_list" class="um_mailchimp_sync_list"  style="width:100px;">
				<option value=""><?php _e('All Lists', 'twodayssss' ); ?></option>
			...
			<a href="#" id="btn_um_mailchimp_sync_now" class="um-btn-mailchimp-progress-start button <# if( data.button_disabled ) { #>disabled<# } #>"><?php _e('Sync Now','twodayssss'); ?></a>
			<# if( data.message ) { #>
		...
		<div class="um_mailchimp_metabox">
			<# if( ! data.step ) { #>
				<select name="um_mailchimp_user_role" class="um_mailchimp_user_role"  style="width:100px;">
					<option value=""><?php _e( 'All Roles', 'twodayssss' ) ?></option>
				...
				<select name="um_mailchimp_user_status" class="um_mailchimp_user_status" style="width:100px;">
					<option value=""><?php _e( 'All Status', 'twodayssss' ) ?></option>
				...
				<a href="javascript:void(0);" id="btn_um_mailchimp_scan_now" class="um-btn-mailchimp-progress-start button <# if( data.button_disabled ) { #>disabled<# } #>"><?php _e('Scan Now','twodayssss'); ?></a>
			<# } else if( data.step == 2 ) { #>
				...
				<a href="javascript:void(0);" id="btn_um_mailchimp_bulk_subscribe" class="um-btn-mailchimp-progress-start button <# if( data.button_disabled ) { #>disabled<# } #>"><?php _e('Opt-in now','twodayssss'); ?></a>
			<# } #>
			<# if( data.message ) { #>
				<span class="um-progress-message-area">
					<# if( data.loading ) { #>
    ...
    <p class="sub"><?php _e( 'Sync Profiles','twodayssss' ); ?></p>
	<div id="um-mailchimp-sync-metabox-wrapper"></div>
    <br />
	<p class="sub"><?php _e( 'Bulk Opt-in','twodayssss' ); ?></p>
