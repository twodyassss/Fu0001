...
function um_bbpress_user_add_tab( $tabs ) {
	...
	$tabs['forums']['subnav'] = array(
		'topics'        => __( 'Topics Started', 'twodayssss' ) . '<span>' . $topic_started . '</span>',
		'replies'       => __( 'Replies Created', 'twodayssss' ) . '<span>' . bbp_get_user_reply_count_raw( $user_id ) . '</span>',
		'favorites'     => __( 'Favorites', 'twodayssss' ) . '<span>' . count( bbp_get_user_favorites_topic_ids( $user_id ) ) . '</span>',
		'subscriptions' => __( 'Subscriptions', 'twodayssss' ) . '<span>' . UM()->bbPress_API()->user_subscriptions_count( $user_id ) . '</span>',
	);
