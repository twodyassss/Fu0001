...
function um_profile_completeness_add_field( $fields ) {
	$fields['completeness_bar'] = array(
		'title'             => __( 'Profile Completeness', 'twodayssss' ),
		'metakey'           => 'completeness_bar',
		'type'              => 'text',
		'label'             => __( 'Profile Completeness', 'twodayssss' ),
		'required'          => 0,
		'public'            => 1,
		'editable'          => 0,
		'edit_forbidden'    => 1,
...
function um_completeness_fields( $html, $field_data, $form_data ) {
	...
	<div class="profilec-setup">
		...
		<h3><?php _e( 'Setup Fields','twodayssss' ); ?></h3>

		<div>
			<?php _e( 'Remaining progress:', 'twodayssss'); ?>&nbsp;
		...
		<div class="profilec-data">
			...
			<div class="profilec-inline">
				...
				<p><label><?php _e('Edit allocated progress (%)','twodayssss'); ?></label>
				...
				<p><a href="#" class="profilec-update button-primary"><?php _e('Update','twodayssss'); ?></a> <a href="#" class="profilec-remove button"><?php _e('Remove','twodayssss'); ?></a><span class="spinner" style="display:none;"></span></p>

			</div>

		</div>
		<p <?php if ( empty( $remaining_progress ) ) { ?>style="display: none"<?php } ?>>
			<a href="#" class="profilec-add button"><?php _e( 'Add field', 'twodayssss' ); ?></a>
		</p>
	</div>
	...
	<div class="profilec-field" style="display: none;">
		...
		<p>
			<label for="progress_value">
				<?php _e( 'How much (%) this field should attribute to profile completeness?', 'twodayssss') ?>
			</label>
			<input type="text" name="progress_value" id="progress_value" value="" placeholder="<?php _e( 'Completeness value (%)', 'twodayssss' ) ?>" class="um-forms-field um-long-field" readonly disabled />
		</p>

		<p>
			<a href="#" class="profilec-save button-primary"><?php _e( 'Save', 'twodayssss' ) ?></a>
			<a href="#" class="profilec-cancel button"><?php _e( 'Cancel', 'twodayssss') ?></a>
		</p>
	</div>
