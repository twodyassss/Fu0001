		...
		if (  isset( $_REQUEST['provider'] ) && $_REQUEST['provider'] == 'facebook' ) {
			...
			if ( ! empty( $accessToken ) ) {
				...
				if( empty( $profile_email ) &&  empty( $_POST ) ){
					UM()->form()->add_error( 'user_email' , __('The email field was not returned. This may be because the email was missing, invalid or hasn\'t been confirmed.','twodayssss') );
