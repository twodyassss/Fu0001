...
<form class="um-show-bbp-subscription" method="get" action="">
	...
	<select class="bbp-subscription" name="bbp-subscription" onchange="this.form.submit();">
		<option value="all"><?php echo sprintf( __( "Show all subscriptions (%d)", 'twodayssss' ), count( $subscribed ) ); ?></option>
		<option value="topic" <?php selected( $show_subscription, 'topic', true ); ?> ><?php echo sprintf( __( "Subscribed Topics (%d)", 'twodayssss' ), count( $subscribed_topics ) ); ?></option>
		<option value="forum" <?php selected( $show_subscription, 'forum', true ); ?> ><?php echo sprintf( __( "Subscribed Forums (%d)", 'twodayssss' ), count( $subscribed_forums ) ); ?></option>
	</select>
</form>
...
<?php if ( $loop && $loop->have_posts() ) { ?>
			...
			<div class="um-item">
					...
					<div class="um-item-action">
						<?php if ( $post_type == 'topic' ) { ?>
							<a href="#" class="um-ajax-action um-tip-e"
							   title="<?php _e( 'Unsubscribe', 'twodayssss' ); ?>"
				...
				<div class="um-item-meta">
					<?php if ( $post_type == 'topic' ) { ?>
						...
						<span><?php _e( "Voices", 'twodayssss' ); ?>
							: <?php echo bbp_get_topic_voice_count( $topic_id ); ?></span>
						<span><?php _e( "Replies", 'twodayssss' ); ?>
					...
					<?php } elseif ( $post_type == 'forum' ) { ?>
						...
						<?php echo ( bbp_get_topic_last_active_time( $forum_id ) ) ? '<span>' . sprintf( __( 'Last active %s', 'twodayssss' ), bbp_get_topic_last_active_time( $forum_id ) ) . '</span>' : ''; ?>
					<?php } ?>
				</div>
			</div>
	...
	<div class="um-profile-note">
		<span><?php echo ( um_profile_id() == get_current_user_id() ) ? __( 'You are not currently subscribed to any topics.', 'twodayssss' ) : __( 'This user is not currently subscribed to any topics.', 'twodayssss' ); ?></span>
	</div>
