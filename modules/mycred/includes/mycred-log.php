...
<?php
		...
		public function pagination( $location = 'top', $id = '' ) {
			...
			if ( $this->have_entries() ) {
				...
				$output = '<span class="displaying-num">' . sprintf( __( 'Showing %d %s', 'twodayssss' ), $total_number, _n( 'entry', 'entries', $total_number, 'twodayssss' ) ) . '</span>';
			}
		...
		public function filter_options( $is_profile = false, $refs = array() ) {
			...
			if ( $this->core->can_edit_creds() && ! $is_profile && $this->num_rows > 0 ) {
				echo '<input type="text" class="form-control" name="user_id" id="myCRED-user-filter" size="12" placeholder="' . __( 'User ID', 'twodayssss' ) . '" value="' . ( ( isset( $_GET['user_id'] ) ) ? $_GET['user_id'] : '' ) . '" /> ';
				$show = true;
			}
			...
			if ( $this->num_rows > 0 ) {
				...
				$options = array( 'ASC' => __( 'Ascending', 'twodayssss' ), 'DESC' => __( 'Descending', 'twodayssss' ) );
			...
			if ( $show === true )
				echo '<input type="submit" class="btn btn-default button button-secondary button-large" value="' . __( 'Filter', 'twodayssss' ) . '" />';
			...
			foreach ( $this->headers as $column_id => $column_name ) {
				switch ( $column_id ) {
					...
					case 'column-username' :
						...
						if ( $user === false )
							$content = '<span>' . __( 'User Missing', 'twodayssss' ) . ' (ID: ' . $log_entry->user_id . ')</span>';
						else
...
' ) . '</style>';
		...
		public function search() {
			...
			<p class="search-box">
				<label class="screen-reader-text" for=""><?php _e( 'Search Log', 'twodayssss' ); ?>:</label>
				<input type="search" name="s" value="<?php echo $serarch_string; ?>" placeholder="<?php _e( 'search log entries', 'twodayssss' ); ?>" />
				<input type="submit" name="mycred-search-log" id="search-submit" class="button button-medium button-secondary" value="<?php _e( 'Search Log', 'twodayssss' ); ?>" />
			</p>
<?php
		}
