...
class Groups_Taxonomies {
	...
	function change_group_featured_image_text( $content ) {
		if ( 'um_groups' === get_post_type() ) {
			$content = str_replace( 'Set featured image', __( 'Set Group Image', 'twodayssss' ), $content );
			$content = str_replace( 'Remove featured image', __( 'Remove Group Image', 'twodayssss' ), $content );
		}
	...
	function change_featured_image_metabox_title() {
		...
		add_meta_box( 'postimagediv', __( 'Group Image', 'twodayssss' ), 'post_thumbnail_meta_box', 'um_groups', 'side' );
	}
}