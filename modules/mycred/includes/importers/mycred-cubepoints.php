	...
	class myCRED_Importer_CubePoints extends WP_Importer {
		...
		public function check_cubepoints() {
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '{$cubepoints}';" ) != $cubepoints ) {
				echo '<div class="error notice notice-error"><p>' . __( 'Could not find a CubePoints installation.', 'twodayssss' ) . '</p></div>';
				return false;
			}
		...
		public function import() {
			...
			if ( $this->imported == 0 ) {
				...
				echo '<div class="error notice notice-error is-dismissible"><p>' . ( ( $action == 'balance' ) ? __( 'No balances were imported.', 'twodayssss' ) : __( 'No log entries were imported!', 'twodayssss' ) ) . '</p></div>';
		...
		public function header() {
			...
			$label = __( 'Import CubePoints', 'twodayssss' );
...
<form id="import-setup" method="post" action="<?php echo esc_attr( wp_nonce_url( $action_url, 'import-upload' ) ); ?>">
	<table class="form-table">
		<tbody>
			<tr>
				<th>
					<label for="import-action"><?php _e( 'Import', 'twodayssss' ); ?></label>
				</th>
				<td>
					<select name="action" id="import-action">
						<option value=""><?php _e( 'Select what to import', 'twodayssss' ); ?></option>
						<option value="log"><?php _e( 'Log Entries Only', 'twodayssss' ); ?></option>
						<option value="balance"><?php _e( 'CubePoints Balances Only', 'twodayssss' ); ?></option>
						<option value="both"><?php _e( 'Log Entries and Balances', 'twodayssss' ); ?></option>
					</select>
				</td>
			</tr>
			<tr>
				<th>
					<label for="mycred-type"><?php _e( 'Point Type', 'twodayssss' ); ?></label>
				</th>
				<td>
