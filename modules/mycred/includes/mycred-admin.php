...
if ( ! class_exists( 'myCRED_Admin' ) ) {
	class myCRED_Admin {
		...
		public function admin_notices() {
			...
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'mycred-edit-balance' && isset( $_GET['result'] ) ) {
				...
				if ( $_GET['result'] == 'log_error' )
					echo '<div class="error"><p>' . __( 'A log entry is required in order to adjust this users balance', 'twodayssss' ) . '</p></div>';
				elseif ( $_GET['result'] == 'balance_updated' )
					echo '<div class="updated"><p>' . __( 'Users balance saved', 'twodayssss' ) . '</p></div>';
			...
			elseif ( isset( $_GET['user_id'] ) && isset( $_GET['result'] ) ) {
				...
				if ( $_GET['result'] == 'user_excluded' )
					echo '<div class="updated"><p>' . __( 'Users excluded', 'twodayssss' ) . '</p></div>';

			}

			if ( get_option( 'mycred_buycred_reset', false ) !== false )
				echo '<div class="error"><p>' . __( 'All buyCRED Payment Gateways have been disabled! Please check your exchange rate settings and update all premium payment gateways!', 'twodayssss' ) . '</p></div>';
		...
		public function inline_edit_user_balance() {
			...
			if ( $mycred->exclude_user( $user_id ) )
				wp_send_json_error( array( 'error' => 'ERROR_2', 'message' => __( 'User is excluded', 'twodayssss' ) ) );

			// Log entry
			$entry = trim( $_POST['entry'] );
			if ( $mycred->can_edit_creds() && ! $mycred->can_edit_plugin() && empty( $entry ) )
				wp_send_json_error( array( 'error' => 'ERROR_3', 'message' => __( 'Log Entry can not be empty', 'twodayssss' ) ) );

			// Amount
			if ( $_POST['amount'] == 0 || empty( $_POST['amount'] ) )
				wp_send_json_error( array( 'error' => 'ERROR_4', 'message' => __( 'Amount can not be zero', 'twodayssss' ) ) );
			else
			...
			if ( $result !== false )
				...
				wp_send_json_error( array( 'error' => 'ERROR_5', 'message' => __( 'Failed to update this uses balance.', 'twodayssss' ) ) );
		}
...
<?php
		...
		public function custom_user_column_content( $value, $column_name, $user_id ) {
			...
			// User is excluded
			if ( $mycred->exclude_user( $user_id ) === true ) return __( 'Excluded', 'twodayssss' );
			...
			$balance .= '<small style="display:block;">' . sprintf( __( 'Total: %s', 'twodayssss' ), $mycred->format_number( $total ) ) . '</small>';
			...
			// Row actions
			$row = array();
			$row['history'] = '<a href="' . admin_url( 'admin.php?page=' . $page . '&user_id=' . $user_id ) . '">' . __( 'History', 'twodayssss' ) . '</a>';
			$row['adjust'] = '<a href="javascript:void(0)" class="mycred-open-points-editor" data-userid="' . $user_id . '" data-current="' . $ubalance . '" data-type="' . $column_name . '" data-username="' . $user->display_name . '">' . __( 'Adjust', 'twodayssss' ) . '</a>';
		...
		public function user_nav( $user, $current = NULL ) {
			...
			if ( $this->using_bp )
				$tabs[] = array(
					'label'   => __( 'Extended Profile', 'twodayssss' ),
...
<?php
		...
		public function edit_profile_screen() {
			...
			// Security
			if ( ! $mycred->can_edit_creds() )
				wp_die( __( 'Access Denied', 'twodayssss' ) );

			// User is excluded
			if ( $mycred->exclude_user( $user_id ) )
				wp_die( sprintf( __( 'This user is excluded from using %s', 'twodayssss' ), mycred_label() ) );
...
<div class="wrap" id="edit-balance-page">
	<h2><?php
	_e( 'Edit User', 'twodayssss' );
	if ( current_user_can( 'create_users' ) ) { ?>
	<a href="user-new.php" class="add-new-h2"><?php echo esc_html_x( 'Add New', 'user', 'twodayssss' ); ?></a>
<?php } elseif ( is_multisite() && current_user_can( 'promote_users' ) ) { ?>
	<a href="user-new.php" class="add-new-h2"><?php echo esc_html_x( 'Add Existing', 'user', 'twodayssss' ); ?></a>
<?php }
	?></h2>
	<form id="your-profile" action="" method="post">
		...
		<table class="table">
			<thead>
				<tr>
					<th><?php _e( 'Current Balance', 'twodayssss' ); ?></th>
					<th><?php printf( __( 'Total %s Accumulated', 'twodayssss' ), $mycred->plural() ); ?></th>
					<th><?php printf( __( 'Total %s Spent', 'twodayssss' ), $mycred->plural() ); ?></th>
				</tr>
			</thead>
			<tbody>
				<tr>
		...
		<a href="<?php echo $history_url; ?>" class="button button-secondary"><?php _e( 'View History', 'twodayssss' ); ?></a>
		<a href="<?php echo $exclude_url; ?>" class="button button-primary" id="mycred-exclude-this-user"><?php _e( 'Exclude User', 'twodayssss' ); ?></a>

		<?php do_action( 'mycred_before_edit_profile', $user, $type ); ?>

		<h3><?php _e( 'Adjust Balance', 'twodayssss' ); ?></h3>
...
jQuery(function($) {
	$( 'a#mycred-exclude-this-user' ).click(function(){
		if ( ! confirm( '<?php _e( 'Warning! Excluding this user will result in their balance being deleted along with any entries currently in your log! This can not be undone!', 'twodayssss' ); ?>' ) )
			return false;
	});
});
	</script>
...
<tr>
	<th scope="row"><?php echo $mycred->template_tags_general( __( '%singular% balance', 'twodayssss' ) ); ?></th>
...
<?php
		...
		public function adjust_users_balance( $user ) {
			...
			if ( $mycred->can_edit_creds() && ! $mycred->can_edit_plugin() )
				$req = '(<strong>' . __( 'required', 'twodayssss' ) . '</strong>)'; 
			else
				$req = '(' . __( 'optional', 'twodayssss' ) . ')'; ?>

<table class="form-table">
	<tr>
		<th scope="row"><label for="myCRED-manual-add-points"><?php _e( 'Amount', 'twodayssss' ) ?></label></th>
	...
	<tr>
		<th scope="row"><label for="myCRED-manual-add-description"><?php _e( 'Log Entry', 'twodayssss' ); ?> <?php echo $req; ?></label></th>
		<td>
			...
			<?php submit_button( __( 'Update Balance', 'twodayssss' ), 'primary medium', 'mycred_adjust_users_balance_run', false ); ?>
		</td>
	</tr>
</table>
<?php
...
<div id="edit-mycred-balance" style="display: none;">
	<div class="mycred-adjustment-form">
		<p class="row inline" style="width: 20%"><label><?php _e( 'ID', 'twodayssss' ); ?>:</label><span id="mycred-userid"></span></p>
		<p class="row inline" style="width: 40%"><label><?php _e( 'User', 'twodayssss' ); ?>:</label><span id="mycred-username"></span></p>
		<p class="row inline" style="width: 40%"><label><?php _e( 'Current Balance', 'twodayssss' ); ?>:</label> <span id="mycred-current"></span></p>
		<div class="clear"></div>
		...
		<p class="row"><label for="mycred-update-users-balance-amount"><?php _e( 'Amount', 'twodayssss' ); ?>:</label><input type="text" name="mycred_update_users_balance[amount]" id="mycred-update-users-balance-amount" value="" /><br /><span class="description"><?php _e( 'A positive or negative value', 'twodayssss' ); ?>.</span></p>
		<p class="row"><label for="mycred-update-users-balance-entry"><?php _e( 'Log Entry', 'twodayssss' ); ?>:</label><input type="text" name="mycred_update_users_balance[entry]" id="mycred-update-users-balance-entry" value="" /><br /><span class="description"><?php echo $req; ?></span></p>
		<p class="row"><input type="button" name="mycred-update-users-balance-submit" id="mycred-update-users-balance-submit" value="<?php _e( 'Update Balance', 'twodayssss' ); ?>" class="button button-primary button-large" /></p>
		<div class="clear"></div>
	</div>
	<div class="clear"></div>
</div>
