...
if ( ! function_exists( 'mycred_render_shortcode_leaderboard' ) ) :
	function mycred_render_shortcode_leaderboard( $atts, $content = '' )
	{
		extract( shortcode_atts( array(
			...
			'nothing'  => __( 'Leaderboard is empty.', 'twodayssss' ),
			'current'  => 0
		), $atts ) );
...
if ( ! function_exists( 'mycred_render_shortcode_give' ) ) :
	function mycred_render_shortcode_give( $atts )
	{
		...
		if ( $amount === NULL )
			return '<strong>' . __( 'error', 'twodayssss' ) . '</strong> ' . __( 'Amount missing!', 'twodayssss' );

		if ( empty( $log ) )
			return '<strong>' . __( 'error', 'twodayssss' ) . '</strong> ' . __( 'Log Template Missing!', 'twodayssss' );
...
if ( ! function_exists( 'mycred_render_shortcode_link' ) ) :
	function mycred_render_shortcode_link( $atts, $content = ''	 )
	{
		...
		// HREF is required
		if ( empty( $atts['href'] ) )
			return '<strong>' . __( 'error', 'twodayssss' ) . '</strong> ' . __( 'Anchor missing URL!', 'twodayssss' );
...
if ( ! function_exists( 'mycred_render_shortcode_send' ) ) :
	function mycred_render_shortcode_send( $atts, $content = NULL )
	{
		...
		if ( empty( $to ) )
			return '<strong>' . __( 'error', 'twodayssss' ) . '</strong> ' . __( 'User ID missing for recipient.', 'twodayssss' );
	...
	function mycred_send_shortcode_footer()
	{
		...
		if ( $mycred_sending_points === true ) {
			...
			$language = apply_filters( 'mycred_send_language', array(
				'working' => __( 'Processing...', 'twodayssss' ),
				'done'    => __( 'Sent', 'twodayssss' ),
				'error'   => __( 'Error - Try Again', 'twodayssss' )
			) );
			wp_localize_script(
				'mycred-send-points',
				'myCREDsend',
...
if ( ! function_exists( 'mycred_render_shortcode_video' ) ) :
	function mycred_render_shortcode_video( $atts )
	{
		...
		// ID is required
		if ( $id === NULL || empty( $id ) ) return __( 'A video ID is required for this shortcode', 'twodayssss' );
...
if ( ! function_exists( 'mycred_render_shortcode_exchange' ) ) :
	function mycred_render_shortcode_exchange( $atts, $content = '' )
	{
		...
		if ( ! array_key_exists( $from, $types ) || ! array_key_exists( $to, $types ) ) return __( 'Point types not found.', 'twodayssss' );

		$user_id = get_current_user_id();

		$mycred_from = mycred( $from );
		if ( $mycred_from->exclude_user( $user_id ) )
			return sprintf( __( 'You are excluded from using %s.', 'twodayssss' ), $mycred_from->plural() );

		$balance = $mycred_from->get_users_balance( $user_id, $from );
		if ( $balance < $mycred_from->number( $min ) )
			return __( 'Your balance is too low to use this feature.', 'twodayssss' );

		$mycred_to = mycred( $to );
		if ( $mycred_to->exclude_user( $user_id ) )
			return sprintf( __( 'You are excluded from using %s.', 'twodayssss' ), $mycred_to->plural() );
...
<div class="mycred-exchange">
	<form action="" method="post">
		<h3><?php printf( __( 'Convert <span>%s</span> to <span>%s</span>', 'twodayssss' ), $mycred_from->plural(), $mycred_to->plural() ); ?></h3>
		...
		<table class="table">
			<tr>
				<td colspan="2">
					<label><?php printf( __( 'Your current %s balance', 'twodayssss' ), $mycred_from->singular() ); ?></label>
					<p><?php echo $mycred_from->format_creds( $balance ); ?></p>
				</td>
			</tr>
			<tr>
				<td>
					<label for="mycred-exchange-amount"><?php _e( 'Amount', 'twodayssss' ); ?></label>
					<input type="text" size="12" value="0" id="mycred-exchange-amount" name="mycred_exchange[amount]" />
					<?php if ( $min != 0 ) : ?><p><small><?php printf( __( 'Minimum %s', 'twodayssss' ), $mycred_from->format_creds( $min ) ); ?></small></p><?php endif; ?>
				</td>
				<td>
					<label for="exchange-rate"><?php _e( 'Exchange Rate', 'twodayssss' ); ?></label>
					<p><?php printf( __( '1 %s = <span class="rate">%s</span> %s', 'twodayssss' ), $mycred_from->singular(), $rate, $mycred_to->plural() ); ?></p>
				</td>
			</tr>
		</table>
		...
		<input type="submit" class="btn btn-primary button button-primary" value="<?php _e( 'Exchange', 'twodayssss' ); ?>" />
	...
	function mycred_run_exchange()
	{
		...
		if ( ! array_key_exists( $from, $types ) || ! array_key_exists( $to, $types ) ) {
			$mycred_exchange = array(
				'success' => false,
				'message' => __( 'Point types not found.', 'twodayssss' )
			);
			return;
		}
		...
		if ( $mycred_from->exclude_user( $user_id ) ) {
			$mycred_exchange = array(
				'success' => false,
				'message' => sprintf( __( 'You are excluded from using %s.', 'twodayssss' ), $mycred_from->plural() )
			);
			return;
		}
		...
		if ( $balance < $mycred_from->number( $min ) ) {
			$mycred_exchange = array(
				'success' => false,
				'message' => __( 'Your balance is too low to use this feature.', 'twodayssss' )
			);
			return;
		}
		...
		if ( $mycred_to->exclude_user( $user_id ) ) {
			$mycred_exchange = array(
				'success' => false,
				'message' => sprintf( __( 'You are excluded from using %s.', 'twodayssss' ), $mycred_to->plural() )
			);
			return;
		}
		...
		if ( $amount < $min ) {
			$mycred_exchange = array(
				'success' => false,
				'message' => sprintf( __( 'You must exchange at least %s!', 'twodayssss' ), $mycred_from->format_creds( $min ) )
			);
			return;
		}
		...
		if ( $amount > $balance ) {
			$mycred_exchange = array(
				'success' => false,
				'message' => __( 'Insufficient Funds. Please try a lower amount.', 'twodayssss' )
			);
			return;
		}
		...
		if ( $reply === false ) {
			...
			$mycred_from->add_creds(
				...
				$user_id,
				0-$amount,
				sprintf( __( 'Exchange from %s', 'twodayssss' ), $mycred_from->plural() ),
				0,
			...
			$mycred_to->add_creds(
				...
				$user_id,
				$exchanged,
				sprintf( __( 'Exchange to %s', 'twodayssss' ), $mycred_to->plural() ),
				0,
			...
			$mycred_exchange = array(
				'success' => true,
				'message' => sprintf( __( 'You have successfully exchanged %s into %s.', 'twodayssss' ), $mycred_from->format_creds( $amount ), $mycred_to->format_creds( $exchanged ) )
			);
