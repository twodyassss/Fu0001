...
class Mailchimp_Ajax {
	...
	function ajax_test_subscribe() {
		...
		if( !empty( $data['_um_test_email'] ) && is_email( $data['_um_test_email'] ) ) {
			...
			wp_send_json_error( __( 'Please enter email', 'twodayssss' ) );
		}
		...
		if ( ! empty( $_POST['force'] ) ) {
			...
			switch( $action ){
				...
				case 'update':
					...
					if ( !$array || !is_array($array) ) wp_send_json_error( __('User list is empty', 'twodayssss') );
					...
					$queue_message = $action == 'subscribe' ? __('0 new subscribers', 'twodayssss') : __('0 new profile updates', 'twodayssss');
					break;
				case 'unsubscribe':
					...
					$queue_message = __('0 new unsubscribers', 'twodayssss');
					break;
			}
			wp_send_json_success(array(
				'message' => __( 'Done', 'twodayssss' ),
				'queue'   => $queue_message
			));
		}
		wp_send_json_error( __('Wrong queue action', 'twodayssss') );
	}
	...
	function ajax_sync_now() {
		...
		if( count( $users ) ) {
			...
			if( $index == $total ) {
				$message = __('Completed', 'twodayssss');
			} else {
				$message = sprintf( __('Processed... %s', 'twodayssss'), ($index/$total*100) . '%' );
			}
			wp_send_json_success( array(
				'batch_id' => $batch_id,
				'message' => $message,
			...
			wp_send_json_error( __( 'You don\'t have any users', 'twodayssss' ) );
		}
	}
	...
	function ajax_scan_now() {
		...
		wp_send_json_success( array(
			'key' => $action_key,
			'total' => count( $users ),
			'scan_total_message' => sprintf( _n( '%d user was selected', '%d users were selected', count( $users ), 'twodayssss' ), count( $users ) )
		) );
	}
	...
	function ajax_bulk_subscribe() {
		...
		if ( empty( $_POST['list'] ) ) {
			wp_send_json_error( __( 'Empty list ID', 'twodayssss' ) );
		}

		if ( empty( $_POST['key'] ) ) {
			wp_send_json_error( __( 'Empty key', 'twodayssss' ) );
		}
		...
		if ( $batch_id === 0 ) {
			wp_send_json_error( __( 'Empty users list', 'twodayssss' ) );
		}
