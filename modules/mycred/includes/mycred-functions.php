	...
	class myCRED_Settings {
		...
		public function defaults() {
			...
			return array(
				...
				'name'        => array(
					'singular'    => __( 'Point', 'twodayssss' ),
					'plural'      => __( 'Points', 'twodayssss' )
				),
				'before'      => '',
				'after'       => '',
				'caps'        => array(
		...
		public function template_tags_comment( $content = '', $ref_id = NULL, $data = '', $link_target = '' ) {
			...
			$comment_post_title = __( 'Deleted Item', 'twodayssss' );
		...
		public function available_template_tags( $available = array(), $custom = '' ) {
			...
			if ( in_array( 'user', $available ) )
				$template_tags[] = array(
					'title' => __( 'User Related', 'twodayssss' ),
					'url'   => ''
				);
			...
			if ( in_array( 'post', $available ) )
				$template_tags[] = array(
					'title' => __( 'Post Related', 'twodayssss' ),
					'url'   => ''
				);
			...
			if ( in_array( 'comment', $available ) )
				$template_tags[] = array(
					'title' => __( 'Comment Related', 'twodayssss' ),
					'url'   => ''
				);
			...
			if ( in_array( 'widget', $available ) )
				$template_tags[] = array(
					'title' => __( 'Widget Related', 'twodayssss' ),
					'url'   => ''
				);
			...
			if ( in_array( 'amount', $available ) )
				$template_tags[] = array(
					'title' => __( 'Amount Related', 'twodayssss' ),
					'url'   => ''
				);
			...
			if ( in_array( 'video', $available ) )
				$template_tags[] = array(
					'title' => __( 'Video Related', 'twodayssss' ),
					'url'   => ''
				);
			...
			if ( ! empty( $custom ) )
				$custom = ' ' . __( 'and', 'twodayssss' ) . ': ' . $custom;

			return __( 'Available Template Tags:', 'twodayssss' ) . ' ' . implode( ', ', $links ) . $custom . '.';
...
if ( ! function_exists( 'mycred_translate_limit_code' ) ) :
	function mycred_translate_limit_code( $code = '' ) {
		...
		if ( $code == '0/x' || $code == 0 )
			return __( 'No limit', 'twodayssss' );

		$result = '-';
		$check  = explode( '/', $code );
		if ( count( $check ) == 2 ) {

			$per    = __( 'in total', 'twodayssss' );
			if ( $check[1] == 'd' )
				$per = __( 'per day', 'twodayssss' );

			elseif ( $check[1] == 'w' )
				$per = __( 'per week', 'twodayssss' );

			elseif ( $check[1] == 'm' )
				$per = __( 'per month', 'twodayssss' );

			$result = sprintf( _n( 'Maximum once', 'Maximum %d times', $check[0], 'twodayssss' ), $check[0] ) . ' ' . $per;

		}

		elseif ( is_numeric( $code ) ) {

			$result = sprintf( _n( 'Maximum once', 'Maximum %d times', $code, 'twodayssss' ), $code );
...
if ( ! function_exists( 'mycred_ordinal_suffix' ) ) :
	function mycred_ordinal_suffix( $num = 0, $depreciated = true ) {
		...
		$result = sprintf( _x( '%d th', 'e.g. 5 th', 'twodayssss' ), $value );
		if ( $num < 11 || $num > 13 ) {
			switch ( $num % 10 ) {

				case 1 : $result = sprintf( _x( '%d st', 'e.g. 1 st', 'twodayssss' ), $value );
				case 2 : $result = sprintf( _x( '%d nd', 'e.g. 2 nd', 'twodayssss' ), $value );
				case 3 : $result = sprintf( _x( '%d rd', 'e.g. 3 rd', 'twodayssss' ), $value );
...
if ( ! function_exists( 'mycred_locate_template' ) ) :
	function mycred_locate_template( $template_name, $template_path = 'twodayssss', $default_path = '' ) {
