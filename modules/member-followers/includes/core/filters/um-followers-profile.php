...
function um_followers_profile_privacy_options( $options ) {
	$options = array_merge( $options, array(
		'followed' => __( 'Only people I follow can view my profile', 'twodayssss' ),
		'follower' => __( 'Followers', 'twodayssss' )
	) );
...
function um_followers_messaging_privacy_options( $options ) {
	$options['followed'] = __( 'Only people I follow', 'twodayssss' );
	$options['follower'] = __( 'Followers', 'twodayssss' );
	return $options;
}
...
function um_followers_can_view_main( $can_view, $user_id ) {
	if ( ! is_user_logged_in() || get_current_user_id() != $user_id ) {
		$is_private_case_old = UM()->user()->is_private_case( $user_id, __( 'Followers', 'twodayssss' ) );
		$is_private_case = UM()->user()->is_private_case( $user_id, 'follower' );
		if ( $is_private_case || $is_private_case_old ) {
			$can_view = __( 'You must follow this user to view profile', 'twodayssss' );
		}
		$is_private_case_old = UM()->user()->is_private_case( $user_id, __( 'Only people I follow can view my profile', 'twodayssss' ) );
		$is_private_case = UM()->user()->is_private_case( $user_id, 'followed' );
		if ( $is_private_case || $is_private_case_old ) {
			$can_view = __( 'You cannot view this profile because the user has not followed you', 'twodayssss' );
		}
	}
...
function um_followers_private_filter_hook( $default, $option, $user_id ) {
	...
	if ( $option == 'follower' || $option == __( 'Followers', 'twodayssss' ) ) {
		if ( ! UM()->Followers_API()->api()->followed( $user_id, get_current_user_id() ) ) {
			return true;
		}
	}

	if ( $option == 'followed' || $option == __( 'Only people I follow can view my profile', 'twodayssss' ) ) {
...
function um_wall_can_view_follower( $can_view, $profile_id ) {
	...
	if ( $privacy == 3 ) {
		if ( ! UM()->Followers_API()->api()->followed( $profile_id, get_current_user_id() ) ) {
			return __( 'You must follow this user to view their social activity', 'twodayssss' );
		}
	} elseif ( $privacy == 4 ) {
		if ( ! UM()->Followers_API()->api()->followed( get_current_user_id(), $profile_id ) ) {
			return __( 'This user must follow you before you can view their wall', 'twodayssss' );
		}
	}
