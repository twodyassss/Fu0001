...
class Groups_Main_API {
	...
	function __construct() {
		...
		$this->privacy_options = array(
			'public' 	=> __('Public','twodayssss'),
			'public_role' => __( 'Public for Role', 'twodayssss' ),
			'private' 	=> __('Private','twodayssss'),
			'hidden' 	=> __('Hidden','twodayssss')
		);
		...
		$this->join_status = array(
			'pending_admin_review'  => __('Pending Admin Review','twodayssss'),
			'pending_member_review' => __('Pending Member Review','twodayssss'),
			'approved' 				=> __('Approved','twodayssss'),
			'rejected'				=> __('Rejected','twodayssss'),
			'blocked' 				=> __('Blocked','twodayssss'),

		);

		$this->group_roles = array(
			'admin' 	=> __('Administrator','twodayssss'),
			'moderator' => __('Moderator','twodayssss'),
			'member'	=> __('Member','twodayssss'),
			'banned'	=> __('Banned','twodayssss'),
		);

		$this->can_invite = array(
			0 => __('All Group Members','twodayssss'),
			1 => __('Group Administrators & Moderators only','twodayssss'),
			2 => __('Group Administrators only','twodayssss')
		);

		$this->group_posts_moderation_options = array(
			'auto-published' => __('Auto Published','twodayssss'),
			'require-moderation' => __('Require Mod/Admin','twodayssss')
		);

		$this->group_members_order = array(
			'asc' => __('Oldest members first','twodayssss'),
			'desc' => __('Newest members first','twodayssss')
		);
	...
	function join_group( $user_id = null, $invited_by_user_id = null, $group_id = null, $group_role = 'member', $new_group = false ) {
		...
		if ( ! in_array( $has_joined, array( 'rejected', '' ) ) ) {
			return array( 'status' => false, 'message' => __( 'You\'re already a member of this group.', 'twodayssss' ) );
		} else {
			$group_privacy = $this->get_privacy_slug( $group_id );
			if($group_privacy === 'public_role') {
				...
				$group_privacy       = ( $array_intersect > 0 ) ? 'public' : 'private';
			}
			switch( $group_privacy ) {
				...
				case 'hidden':
					...
					return array('status' => false, 'privacy' => $group_privacy, 'labels' => $this->privacy_groups_button_labels[ $group_privacy ],'message' => __('Only members can add you to this group.','twodayssss') );
					break;
				case 'private':
				...
				foreach ( $members as $key => $member ) {
					...
					if ( 'approved' == $status ) {
						$data['data'][] =  array( 'user' =>
							                         ...
							                         .__('Already a member','twodayssss')
							                         .'</span>'
						);
	...
	function search_member_suggest() {
		...
		if ( $wpdb->num_rows > 0  ) {
			...
			foreach ( $users as $user_id ) {
				...
				if ( $has_joined ) {
					$user_found = sprintf(__('<strong>%s - %s</strong> - already a member','twodayssss'), um_user('user_email'), um_user('user_login') );
				} else {
					$user_found = sprintf(__('<strong>%s - %s</strong>','twodayssss'), um_user('user_email'), um_user('user_login') );
				}
		...
		return wp_send_json(__('Nothing found','twodayssss'));
	}
	...
	public function get_groups_button_labels( $privacy = null ) {
		...
		if ( empty( $this->privacy_groups_button_labels ) || $privacy ) {
			$this->privacy_groups_button_labels = array(
					'public'	 => array(
							'join'	 => __( 'Join Group', 'twodayssss' ),
							'leave'	 => __( 'Leave Group', 'twodayssss' ),
							'hover'	 => '',
					),
					'private'	 => array(
							'join'	 => __( 'Join Group', 'twodayssss' ),
							'leave'	 => __( 'Request Sent', 'twodayssss' ),
							'_leave' => __( 'Leave Group', 'twodayssss' ),
							'hover'	 => __( 'Cancel', 'twodayssss' ),
					),
					'public_role'	 => array(
							'join'	 => __( 'Join Group', 'twodayssss' ),
							'leave'	 => __( 'Request Sent', 'twodayssss' ),
