...
class Activity_Main_API {
	...
	function __construct() {
		$this->global_actions['status'] = __( 'New wall post', 'twodayssss' );
		$this->global_actions['new-user'] = __( 'New user', 'twodayssss' );
		$this->global_actions['new-post'] = __( 'New blog post', 'twodayssss' );
		$this->global_actions['new-product'] = __( 'New product', 'twodayssss' );
		$this->global_actions['new-gform'] = __('New Gravity Form','twodayssss');
		$this->global_actions['new-gform-submission'] = __('New Gravity Form Answer','twodayssss');
		$this->global_actions['new-follow'] = __( 'New follow', 'twodayssss' );
		$this->global_actions['new-topic'] = __( 'New forum topic', 'twodayssss' );
	...
	function shorten_string( $string )
	{
		...
		if (count( $array ) <= $wordsreturned) {
			...
			$retval = implode( " ", $array ) . " <span class='um-activity-seemore'>(<a href='' class='um-link'>" . __( 'See more', 'twodayssss' ) . "</a>)</span>" . " <span class='um-activity-hiddentext'>" . implode( " ", $res ) . "</span>";
		}
	...
	function set_url_meta( $url, $post_id ) {
		...
		if (isset( $tags['title'] )) {
			...
			$content = str_replace( '{post_title}', '<span class="post-title">' . __( 'Untitled', 'twodayssss' ) . '</span>', $content );
		}
	...
	function can_view_wall( $profile_id ) {
		...
		if ( ! is_user_logged_in() ) {
			if ( UM()->options()->get( 'activity_require_login' ) ) {
				$can_view = __( 'You must login to view this user activity', 'twodayssss' );
			} elseif ( $privacy == 1 ) {
				$can_view = __( 'Please login to view this user\'s activity', 'twodayssss' );
			} elseif ( $privacy == 2 ) {
				$can_view = __( 'This user wall is private', 'twodayssss' );
			}
		} else {
	...
	function human_time_diff( $from, $to = '' )
	{
		...
		if ($diff < 60) {
			...
			if ($mins <= 1)
				$mins = 1;
			if ($mins == 1) {
				$since = sprintf( __( '%s min', 'twodayssss' ), $mins );
			} else {
				$since = sprintf( __( '%s mins', 'twodayssss' ), $mins );
			}
			...
			if ($hours <= 1)
				$hours = 1;
			if ($hours == 1) {
				$since = sprintf( __( '%s hr', 'twodayssss' ), $hours );
			} else {
				$since = sprintf( __( '%s hrs', 'twodayssss' ), $hours );
			}
			...
			if ($days <= 1)
				$days = 1;
			if ($days == 1) {
				$since = sprintf( __( 'Yesterday at %s', 'twodayssss' ), date_i18n( 'g:ia', $from ) );
			} else {
				$since = sprintf( __( '%s at %s', 'twodayssss' ), date_i18n( 'F d', $from ), date_i18n( 'g:ia', $from ) );
			}
			...
			$since = sprintf( __( '%s at %s', 'twodayssss' ), date_i18n( 'F d, Y', $from ), date_i18n( 'g:ia', $from ) );
	...
	function get_faces( $post_id, $num = 10 ) {
		...
		return '<a href="#" data-post_id="' . esc_attr( $post_id ) . '" class="um-activity-show-likes um-tip-s" title="' . esc_attr__( 'People who like this', 'twodayssss' ) . '">' . $res . '</a>';
	}
	...
	function ajax_like_comment() {
		...
		if (!is_user_logged_in())
			$output['error'] = __( 'You must login to like', 'twodayssss' );

		if (!isset( $_POST['commentid'] ) || !is_numeric( $_POST['commentid'] ))
			$output['error'] = __( 'Invalid comment', 'twodayssss' );
	...
	function ajax_unlike_comment() {
		...
		if (!is_user_logged_in())
			$output['error'] = __( 'You must login to unlike', 'twodayssss' );
	...
	function ajax_like_post() {
		...
		if (!isset( $_POST['postid'] ) || !is_numeric( $_POST['postid'] ))
			$output['error'] = __( 'Invalid wall post', 'twodayssss' );
	...
	function ajax_wall_comment() {
		...
		if ( ! is_user_logged_in() ) {
			$output['error'] = __( 'Login to post a comment', 'twodayssss' );
		}
		...
		if ( ! isset( $_POST['comment'] ) || trim( $_POST['comment'] ) == '' ) {
			$output['error'] = __( 'Enter a comment first', 'twodayssss' );
		}
	...
	function ajax_activity_publish() {
		...
		if ( ! is_user_logged_in() ) {
			$output['error'] = __( 'You can not post as guest', 'twodayssss' );
		}

		if ( $_post_content == '' || trim( $_post_content ) == '' ) {
			if ( trim( $_post_img ) == '' ) {
				$output['error'] = __( 'You should type something first', 'twodayssss' );
			}
		}
...
//				$args['post_content'] = apply_filters( 'um_activity_insert_post_content_filter', $args['post_content'], get_current_user_id(), $post_id, 'new' );
	...
	function ajax_get_activity_post() {
		...
		if ( empty( $post_id ) ) {
			$output['error'] = __( 'You should select post first', 'twodayssss' );
		}
	...
	function ajax_get_activity_comment() {
		...
		if ( empty( $comment_id ) ) {
			$output['error'] = __( 'You should select comment first', 'twodayssss' );
		}
