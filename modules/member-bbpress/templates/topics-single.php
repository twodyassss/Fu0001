...
while ( $loop->have_posts() ) {
	...
	<div class="um-item">
		<div class="um-item-link"><?php if ( bbp_get_topic_status( $topic_id ) == 'closed' ) { ?><i class="um-faicon-lock" title="<?php _e( 'Locked', 'twodayssss' ); ?>"></i><?php } ?><a href="<?php the_permalink(); ?>"><?php bbp_topic_title( $topic_id ); ?></a></div>
...
if ( isset( $modified_args ) && $loop->have_posts() && $loop->found_posts >= 10 ) { ?>
	...
	<div class="um-load-items">
		<a href="#" class="um-ajax-paginate um-button" data-hook="um_bbpress_load_topics" data-args="<?php echo $modified_args; ?>"><?php _e( 'load more topics', 'twodayssss' ); ?></a>
	</div>
