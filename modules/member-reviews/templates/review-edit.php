...
<div class="um-reviews-item" data-review_id="<?php echo esc_attr( $post->ID ); ?>" data-user_id="<?php echo esc_attr( um_profile_id() ); ?>">
	...
	<div class="um-reviews-post review-list">
		...
		<span class="um-reviews-meta"><?php printf( __( 'by <a href="%s">%s</a>, %s', 'twodayssss' ), um_user_profile_url(), um_user( 'display_name' ), get_the_time( UM()->options()->get( 'review_date_format' ) ) ); ?></span>

		<span class="um-reviews-content"><?php echo nl2br( $content ); ?></span>

		<?php if( UM()->Reviews_API()->api()->is_flagged( $post->ID ) ) { ?>
			<div class="um-reviews-flagged"><?php esc_html_e( 'This is currently being reviewed by an admin', 'twodayssss' ); ?></div>
		<?php } ?>

		<?php if( UM()->Reviews_API()->api()->is_pending( $post->ID ) ) { ?>
			<div class="um-reviews-pending"><?php esc_html_e( 'This review will be moderated by an admin before it is live.', 'twodayssss' ); ?></div>
		<?php } ?>
	...
	<div class="um-reviews-post review-form">
		...
		<form class="um-reviews-form" action="" method="post">
			...
			<span class="um-reviews-title">
				<input type="text" name="title" placeholder="<?php esc_attr_e( 'Enter subject...', 'twodayssss' ); ?>" value="<?php echo esc_attr( $post->post_title ); ?>" maxlength="60" />
			</span>

			<span class="um-reviews-meta"><?php printf( __( 'by <a href="%s">%s</a>, %s', 'twodayssss' ), um_user_profile_url(), um_user( 'display_name' ), current_time( UM()->options()->get( 'review_date_format' ) ) ); ?></span>

			<span class="um-reviews-content">
				<textarea name="content" placeholder="<?php esc_attr_e( 'Enter your review...', 'twodayssss' ); ?>"><?php echo esc_textarea( $content ); ?></textarea>
			</span>
			...
			<span class="um-reviews-send"><input type="submit" value="<?php _e( 'Save Review', 'twodayssss' ); ?>" class="um-button" /></span>
