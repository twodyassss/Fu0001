...
?>
		<div class="box">
			<label for="<?php echo $id; ?>"><?php echo $mycred->template_tags_general( __( 'Reward with %plural%', 'twodayssss' ) ); ?></label>
			<input type="text" name="_mycred_reward[<?php echo $variation->ID; ?>][<?php echo $point_type; ?>]" id="<?php echo $id; ?>" class="input-text" placeholder="<?php _e( 'Leave empty for no rewards', 'twodayssss' ); ?>" value="<?php echo esc_attr( $prefs[ $point_type ] ); ?>" />
		</div>
<?php
...
if ( ! function_exists( 'mycred_woo_payout_rewards' ) ) :
	function mycred_woo_payout_rewards( $order_id ) {
		...
		if ( $paid_with == 'twodayssss' && apply_filters( 'mycred_woo_reward_mycred_payment', false, $order ) === false )
			return;
...
function mycred_register_woocommerce_hook( $installed ) {
	...
	$installed['wooreview'] = array(
		'title'         => __( 'WooCommerce Product Reviews', 'twodayssss' ),
		'description'   => __( 'Awards %_plural% for users leaving reviews on your WooCommerce products.', 'twodayssss' ),
		'documentation' => '',
...
<div class="hook-instance">
	<div class="row">
		...
		<div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
			<div class="form-group">
				<label for="<?php echo $this->field_id( 'limit' ); ?>"><?php _e( 'Limit', 'twodayssss' ); ?></label>
