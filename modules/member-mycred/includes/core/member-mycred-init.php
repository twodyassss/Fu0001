...
class UM_myCRED_API {
	...
	function __construct() {
		...
		add_action( 'init', array( &$this,'load_hooks' ), 2 );
	 ...
	 */
	function enqueue() {
		require_once um_mycred_path . 'includes/core/class-mycred-enqueue.php';
	 ...
	 */
	function account() {
		require_once um_mycred_path . 'includes/core/class-mycred-account.php';
	...
	***/
	...
	function show_badges_all( $template = 1 ) {
		...
		if ( $template == 1 ) {
			...
			if ( ! empty( $all_badges ) ) {
				$output = twodays_get_template( 'badges.php', um_mycred_plugin, compact(
					'mycred',
					'size',
					'all_badges',
					'point_types',
	...
	***/
	function get_points( $user_id, $value = null ) {
		...
		$value = sprintf( _n( '%s point', '%s points', $value, 'twodayssss' ),  $value );
	...
	***/
	function add( $user_id, $add, $args = array() ) {
		...
		if ( $default_handler ) {
			...
			$log_text = str_replace('%task%', __( $task,'twodayssss') , __($log_template,'twodayssss') );
		...
		$description = sprintf(
			__('Earned %s via Ultimate Member( %s )','twodayssss'),
			'%plural%',
			$action
		);
	...
	***/
	...
	function um_mycred_custom_notification( $content, $vars = array() ){
		...
		$content = str_replace('%task%', __( $task,'twodayssss') , __($notification_template,'twodayssss') );
