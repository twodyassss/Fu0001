<div class="um-admin-metabox">
	...
	if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'edit' ) {
		$fields[] = array(
			'id'		    => 'mailing_list_id',
			'type'		    => 'info_text',
			'label'		    => __( 'Connected to Mailing List ID','twodayssss' ),
		...
		$fields[] = array(
			...
			'type'		    => 'select',
			'size'		    => 'medium',
			'label'		    => __( 'Choose a list','twodayssss' ),
			'tooltip'		=> __('Choose a list from your MailChimp account','twodayssss'),
			'value'		    => '',
			'options'		=> $lists,
		);
	}
	...
	$fields = array_merge( $fields, array(
		array(
			'id'		    => '_um_status',
			'type'		    => 'checkbox',
			'label'		    => __( 'Enable this MailChimp list','twodayssss' ),
			'tooltip'		=> __( 'Turn on or off this list globally. If enabled the list will be available in user account page.','twodayssss' ),
		...
		array(
			...
			'type'		    => 'select',
			'size'		    => 'medium',
			'label'		    => __( 'Enable double opt-in' ,'twodayssss' ),
			'tooltip'		=> __( 'Send contacts an opt-in confirmation email when they subscribe to your list.', 'twodayssss'),
			'value'		    => UM()->query()->get_meta_value('_um_double_optin'),
			'options'		=> array(
                ''    => __( 'Default' ,'twodayssss' ),
                '1' => __( 'Yes' ,'twodayssss' ),
                '0'  => __( 'No' ,'twodayssss' ),
            ),
		),
		array(
			'id'		    => '_um_desc',
			'type'		    => 'text',
			'label'		    => __( 'List Description in Account Page','twodayssss' ),
			'tooltip'		=> __( 'This text will be displayed in Account > Notifications to encourage user to sign or know what this list is about','twodayssss' ),
			'value'		    => UM()->query()->get_meta_value('_um_desc', null, 'na'),
		),
		array(
			'id'		    => '_um_desc_reg',
			'type'		    => 'text',
			'label'		    => __( 'List Description in Registration','twodayssss' ),
			'tooltip'		=> __( 'This text will be displayed in register form if you enable this mailing list to be available during signup','twodayssss' ),
			'value'		    => UM()->query()->get_meta_value('_um_desc_reg', null, 'na'),
		),
		array(
			'id'		    => '_um_reg_status',
			'type'		    => 'checkbox',
			'label'		    => __( 'Automatically add new users to this list', 'twodayssss' ),
			'tooltip'		=> __( 'If turned on users will automatically be subscribed to this when they register. When turned on this list will not show on register form even if you add MailChimp field to register form.','twodayssss' ),
		...
		array(
			...
			'type'		    => 'select',
			'size'		    => 'medium',
			'label'		    => __( 'Which roles can subscribe to this list' ,'twodayssss' ),
			'tooltip'		=> __( 'Select which roles can subscribe to this list. Users who cannot subscribe to this list will not see this list on their account page.', 'twodayssss'),
