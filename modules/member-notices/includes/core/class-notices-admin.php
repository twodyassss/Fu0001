...
class Notices_Admin {
	...
	function enter_title_here( $title ) {
		...
		if ( 'um_notice' == $screen->post_type ) {
			$title = __( 'Enter notice title here', 'twodayssss' );
		}
	...
	function um_extend_admin_menu() {
		add_submenu_page( $this->slug, __( 'Notices', 'twodayssss' ), __( 'Notices', 'twodayssss' ), 'manage_options', 'edit.php?post_type=um_notice', '' );
	}
	...
	function manage_edit_um_notice_columns( $columns ) {
		$columns['shortcode'] = __('Shortcode','twodayssss');
		$columns['reach'] = __('Reach','twodayssss') . UM()->tooltip( __('How many people reached this notice? Count users who seen and closed the notice only','twodayssss') );
		return $columns;
	}
