...
function um_verified_info( $user_id, $args ) {
	...
	if ( $user_status == 'unverified' ) {
		echo '<div class="um-verified-info"><a href="' . UM()->Verified_Users_API()->api()->verify_url( $user_id, um_user_profile_url() ) . '" class="um-link um-verified-request-link">' . __( 'Request Verification', 'twodayssss' ) . '</a></div>';
	} elseif ( $user_status == 'pending' ) {
		$cancel = UM()->Verified_Users_API()->api()->verify_cancel_url( $user_id, um_user_profile_url() );
		echo '<div class="um-verified-info">' . sprintf( __( 'Your verification request is currently pending. <a href="%s" class="um-verified-cancel-request">Cancel request?</a>', 'twodayssss' ), $cancel ) . '</div>';
	}
}
...
function um_verified_account_info( $shortcode_args ) {
	...
	echo '<div class="um-field-label"><label>' . __( 'Get Verified', 'twodayssss' ) . '</label><div class="um-clear"></div></div>';

	$verified_status = UM()->Verified_Users_API()->api()->verified_status( $user_id );

	if ( $verified_status == 'unverified') {
		echo '<div class="um-verified-info"><a href="' . UM()->Verified_Users_API()->api()->verify_url( $user_id, um_get_core_page( 'account' ) ) . '" class="um-link um-verified-request-link">' . __( 'Request Verification', 'twodayssss' ) . '</a></div>';
...
function um_request_verification() {
	...
	if ( isset( $_REQUEST['request_verification'] ) && isset( $_REQUEST['uid'] ) ) {
		...
		if ( $user_id != get_current_user_id() || UM()->Verified_Users_API()->api()->verified_status( $user_id ) != 'unverified' )
			wp_die( __('Unauthorized.','twodayssss') );

		um_fetch_user( $user_id );

		if ( um_user('verified_req_disallowed') )
			wp_die( __('You are not allowed to do this action.','twodayssss') );
