...
function um_social_login_custom_error( $msg, $err_t ) {
	...
	foreach ( $providers as $key => $info ) {
		if ( strstr( $err_t, $key ) && $err_t == $key . '_exist' ) {
			$msg = sprintf( __(' This %s account is already linked to another user.', 'twodayssss' ), $info['name'] );
		}
	}
...
function um_social_login_account_tab( $tabs ) {
	...
	$tabs[450]['social']['title'] = __('Social Connect','twodayssss');
		...
		<?php foreach( $providers as $provider => $array ) { ?>
			...
			<div class="um-provider">
				...
				<div class="um-provider-title">
					<?php printf(__('Connect to %s','twodayssss'), $array['name']); ?>
				...
				<div class="um-provider-conn">
					...
					<?php if ( UM()->Social_Login_API()->is_connected( $user_id, $provider ) ) { ?>
						...
						<div class="um-provider-disconnect">(<a href="<?php echo UM()->Social_Login_API()->disconnect_url( $provider ); ?>"><?php _e( 'Disconnect', 'twodayssss' ) ?></a>)</div>

					<?php } else { ?>

						<a title="<?php printf( esc_attr__('Connect to %s','twodayssss'), $array['name'] ); ?>" href="<?php echo esc_attr( UM()->Social_Login_API()->login_url( $provider ) ); ?>" class="um-social-btn um-social-btn-<?php echo esc_attr( $provider ); ?>">
