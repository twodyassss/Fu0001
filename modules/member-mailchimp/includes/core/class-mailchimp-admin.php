...
class Mailchimp_Admin {
	...
	function add_metabox_form() {
		...
		add_meta_box(
			...
			__( 'Testing connection with Mailchimp server', 'twodayssss' ),
			array( &$this, 'load_metabox_form' ),
			'um_mailchimp',
			'normal',
			'default'
	...
	function admin_notices() {
		...
		if ( !$key ) {
			...
			echo sprintf(__( 'You must add your <strong>MailChimp API</strong> key before connecting your newsletter lists. <a href="%s">Hide this notice</a>','twodayssss'), $hide_link);

			echo '</p>';

			echo '<p><a href="' . admin_url('admin.php?page=um_options&tab=extensions&section=mailchimp' ) . '" class="button button-primary">' . __( 'Setup MailChimp API', 'twodayssss' ) . '</a></p></div>';
	...
	function manage_edit_um_mailchimp_columns( $columns ) {
		...
		$new_columns['title'] = __('Title','twodayssss');
		$new_columns['connection'] = __('Connection','twodayssss');
		$new_columns['status'] = __('Status','twodayssss');
		$new_columns['reg_status'] = __('Automatic Signup','twodayssss');
		$new_columns['list_id'] = __('List ID','twodayssss');
		$new_columns['subscribers'] = __('Subscribers','twodayssss');
		$new_columns['available_to'] = __('Roles','twodayssss');
		...
		switch ( $column_name ) {
			...
			case 'connection':
				...
				if ( isset( $remote_lists[ $list_id ] ) ) {
					echo '<span class="um-adm-ico um-admin-tipsy-n" title="'.__('List found','twodayssss').'"><i class="um-faicon-check"></i></span>';
				} else {
					delete_post_meta( $id, '_um_status');
					echo '<span class="um-adm-ico inactive um-admin-tipsy-n" title="'.__('Unknown list','twodayssss').'"><i class="um-faicon-remove"></i></span>';
				}
				break;
			...
			case 'status':
				...
				if ( $status ) {
					echo '<span class="um-adm-ico um-admin-tipsy-n" title="'.__('Active','twodayssss').'"><i class="um-faicon-check"></i></span>';
				} else {
					echo '<span class="um-adm-ico inactive um-admin-tipsy-n" title="'.__('Inactive','twodayssss').'"><i class="um-faicon-remove"></i></span>';
				}
				break;
	...
	***/
	function metabox_content() {
		...
		wp_localize_script('um-mailchimp-dashboard', 'um_mailchimp_data', array(
			...
			'status_list'           => array(
				'approved'                      => __( 'Approved','twodayssss' ),
				'awaiting_admin_review'         => __( 'Awaiting Admin Review', 'twodayssss' ),
				'awaiting_email_confirmation'   => __( 'Awaiting Email Confirmation', 'twodayssss' ),
				'inactive'                      => __( 'Inactive', 'twodayssss' ),
				'rejected'                      => __( 'Rejected', 'twodayssss' ),
			),
			'labels'                => array(
				'sync_message'                  => __( 'Starting synchronization...', 'twodayssss' ),
				'scan_message'                  => __( 'Checking subscription status...', 'twodayssss' ),
				'start_bulk_subscribe_process'  => __( 'Opt-in users... 0%', 'twodayssss' ),
				'sync_process'                  => __( 'Syncronization...', 'twodayssss' ),
				'processing'                  => __( 'Processing...', 'twodayssss' )
			)
		));
	...
	function enter_title_here( $title ) {
		...
		if ( 'um_mailchimp' == $screen->post_type ) {
			$title = __('e.g. My First Mailing List','twodayssss');
		}
	...
	***/
	function um_extend_admin_menu() {
		...
		add_submenu_page( UM()->admin_menu()->slug, __('MailChimp','twodayssss'), __('MailChimp','twodayssss'), 'manage_options', 'edit.php?post_type=um_mailchimp', '' );
