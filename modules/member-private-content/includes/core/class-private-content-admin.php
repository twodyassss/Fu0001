...
class Private_Content_Admin {
	...
	function add_username( $post ) {
		if ( $post->post_type == 'um_private_content' ) {
			...
			if ( ! empty( $user->user_login ) )
				echo '<h2 style="margin: 0;">' . sprintf( __( 'Private Content for %s', 'twodayssss'), $user->user_login ) . '</h2>';
		}
	}
	...
	function um_admin_user_row_actions( $actions, $user_object ) {
		...
		if ( $private_content_link ) {
			$actions['private-content'] = "<a class='' href='" . $private_content_link . "'>" . __( 'Private Content', 'twodayssss' ) . "</a>";
		}
	...
	function ajax_generate_private_pages() {
		...
		wp_send_json_success( array( 'message' => __( 'Private Content pages was generated successfully', 'twodayssss' ) ) );
