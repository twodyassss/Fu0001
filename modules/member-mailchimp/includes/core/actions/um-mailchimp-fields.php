 ...
 */
	function um_admin_field_edit_hook_mailchimp_list( $val ) {
        ...
        <p><label for="_mailchimp_list"><?php _e('Select a List','twodayssss'); ?> <?php UM()->tooltip( __('You can set up lists or integrations in Ultimate Member > MailChimp','twodayssss') ); ?></label>
...
add_action('um_admin_field_edit_hook_mailchimp_list', 'um_admin_field_edit_hook_mailchimp_list');
	...
	function um_admin_field_edit_hook_mailchimp_auto_subscribe( $val ) {
		...
		<p>
			<label for="_required">
				<?php
				_e('Automatically add users to this list on form submit', 'twodayssss');
				UM()->tooltip( __( 'If turned on users will be subscribed to list on form submit. When turned on this list will not be shown on form.', 'twodayssss' ) );
				?>
			</label>
