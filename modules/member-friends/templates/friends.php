...
$note = ( $user_id == get_current_user_id() ) ? __( 'You do not have any friends yet.', 'twodayssss' ) : __( 'This user does not have any friends yet.', 'twodayssss' );

if ( isset( $_is_reqs ) ) {
	$note = __( 'You do not have pending friend requests yet.', 'twodayssss' );
}

if ( isset( $_sent ) ) {
	$note = __( 'You have not sent any friend requests yet.', 'twodayssss' );
}
		...
		<div class="um-friends-user">
			...
			<div class="um-friends-user-btn">
				<?php if ( $user_id2 == get_current_user_id() ) {
					echo '<a href="' . um_edit_profile_url() . '" class="um-friend-edit um-button um-alt">' . __( 'Edit profile', 'twodayssss' ) . '</a>';
				} else {
