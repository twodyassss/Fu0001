...
<?php
		...
		public function admin_print_styles() {
			...
			if ( ! wp_script_is( 'mycred-admin', 'registered' ) ) {
				...
				wp_localize_script( 'mycred-admin', 'twodayssss', apply_filters( 'mycred_localize_admin', array( 'active' => '-1' ) ) );
			}
		...
		public function admin_page_settings() {
			// Security
			if ( ! current_user_can( 'manage_network_options' ) ) wp_die( __( 'Access Denied', 'twodayssss' ) );
	...
	<div class="wrap" id="myCRED-wrap">
		...
		<h2> <?php echo sprintf( __( '%s Network', 'twodayssss' ), $name ); ?></h2>
		<?php
			...
			if ( isset( $_GET['settings-updated'] ) )
				echo '<div class="updated"><p>' . __( 'Network Settings Updated', 'twodayssss' ) . '</p></div>'; ?>

<p><?php echo sprintf( __( 'Configure network settings for %s.', 'twodayssss' ), $name ); ?></p>
<form method="post" action="<?php echo admin_url( 'options.php' ); ?>" class="">
	<?php settings_fields( 'mycred_network' ); ?>

	<div class="list-items expandable-li" id="accordion">
		<h4><div class="icon icon-inactive core"></div><?php _e( 'Settings', 'twodayssss' ); ?></h4>
		<div class="body" style="display:block;">
			<label class="subheader"><?php _e( 'Master Template', 'twodayssss' ); ?></label>
			<ol id="myCRED-network-settings-enabling">
				<li>
					...
					<label for="myCRED-network-"><?php _e( 'Yes', 'twodayssss' ); ?></label>
				</li>
				<li>
					<input type="radio" name="mycred_network[master]" id="myCRED-network-overwrite-disabled" <?php checked( $prefs['master'], 0 ); ?> value="0" /> 
					<label for="myCRED-network-"><?php _e( 'No', 'twodayssss' ); ?></label>
				</li>
				<li>
					<p class="description"><?php echo sprintf( __( "If enabled, %s will use your main site's settings for all other sites in your network.", 'twodayssss' ), $name ); ?></p>
				</li>
			</ol>
			<label class="subheader"><?php _e( 'Central Logging', 'twodayssss' ); ?></label>
			<ol id="myCRED-network-log-enabling">
				...
				<li>
					<p class="description"><?php echo sprintf( __( "If enabled, %s will log all site actions in your main site's log.", 'twodayssss' ), $name ); ?></p>
				</li>
			</ol>
			<label class="subheader"><?php _e( 'Site Block', 'twodayssss' ); ?></label>
			<ol id="myCRED-network-site-blocks">
				<li>
					...
					<span class="description"><?php echo sprintf( __( 'Comma separated list of blog ids where %s is to be disabled.', 'twodayssss' ), $name ); ?></span>
				</li>
			</ol>
	...
	<p><?php submit_button( __( 'Save Network Settings', 'twodayssss' ), 'primary large', 'submit', false ); ?></p>
</form>	
