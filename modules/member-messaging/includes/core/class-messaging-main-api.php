...
class Messaging_Main_API {
	...
	function get_conversation( $user1, $user2, $conversation_id ) {
		...
		if ( in_array( $conversation_id, $hidden_conversations ) ) { ?>
			<span class="um-message-notice">
				<?php _e( 'This conversation is hidden.', 'twodayssss' ) ?>
			</span>
		<?php } else {
			...
			if ( ! um_user( 'can_read_pm' ) ) {
				...
				if ( um_user( 'can_start_pm' ) ) {
					...
					foreach ( $messages as $message ) {
						twodays_get_template( 'message.php', um_messaging_plugin, array(
							'class'         => 'right_m',
							'status'        => '',
							'message'       => $message,
							'can_remove'    => true,
				...
				<span class="um-message-notice">
					<?php _e( 'Your membership level does not allow you to view conversations.', 'twodayssss' ) ?>
				</span>
			<?php } else {
	...
	function human_time_diff( $from, $to = '' ) {
		...
		if ( $diff < 60 ) {
			$since = sprintf( __('%ss','twodayssss'), $diff );
		} elseif ( $diff < HOUR_IN_SECONDS ) {
			$mins = round( $diff / MINUTE_IN_SECONDS );
			if ( $mins <= 1 )
				$mins = 1;
			/* translators: min=minute */
			$since = sprintf( __('%sm','twodayssss'), $mins );
		} elseif ( $diff < DAY_IN_SECONDS && $diff >= HOUR_IN_SECONDS ) {
			$hours = round( $diff / HOUR_IN_SECONDS );
			if ( $hours <= 1 )
				$hours = 1;
			$since = sprintf( __('%sh','twodayssss'), $hours );
		} elseif ( $diff < WEEK_IN_SECONDS && $diff >= DAY_IN_SECONDS ) {
			$days = round( $diff / DAY_IN_SECONDS );
			if ( $days <= 1 )
				$days = 1;
			$since = sprintf( __('%sd','twodayssss'), $days );
		} elseif ( $diff < 30 * DAY_IN_SECONDS && $diff >= WEEK_IN_SECONDS ) {
			$weeks = round( $diff / WEEK_IN_SECONDS );
			if ( $weeks <= 1 )
				$weeks = 1;
			$since = sprintf( __('%sw','twodayssss'), $weeks );
		} elseif ( $diff < YEAR_IN_SECONDS && $diff >= 30 * DAY_IN_SECONDS ) {
			$months = round( $diff / ( 30 * DAY_IN_SECONDS ) );
			if ( $months <= 1 )
				$months = 1;
			$since = sprintf( __('%sm','twodayssss'), $months );
		} elseif ( $diff >= YEAR_IN_SECONDS ) {
			$years = round( $diff / YEAR_IN_SECONDS );
			if ( $years <= 1 )
				$years = 1;
			$since = sprintf( __('%sy','twodayssss'), $years );
		}
	...
	function ajax_messaging_login_modal() {
		...
		<div class="um-message-modal">
			...
			<div class="um-message-header um-popup-header">
				<div class="um-message-header-left"><?php printf(__('%s Please login to message <strong>%s</strong>','twodayssss'), get_avatar( $message_to, 40 ), um_user('display_name') ); ?></div>
	...
	function ajax_messaging_start() {
		...
		<div class="um-message-modal">
			<?php twodays_get_template( 'conversation.php', um_messaging_plugin, array(
	...
	function ajax_messaging_update() {
		...
		$output["errors"] = array();
		if ( ! isset( $_POST['message_to'] ) || ! is_numeric( $_POST['message_to'] ) || ! is_user_logged_in() ) {
			wp_send_json_error( esc_js( __( "Invalid target user_ID or user is not logged in", 'twodayssss' ) ) );
		}
		...
		if ( ! um_user( 'can_read_pm' ) ) {
			...
			if ( $wpdb->num_rows <= 0 ) {
				...
				wp_send_json_error( esc_js( sprintf( __( "UM Messaging - invalid query: %s", 'twodayssss' ), $last_update_query ) ) );

			} else {
				if ( ! $results[0]->last_updated ) {
					wp_send_json_error( esc_js( __( "UM Messaging - No result found", 'twodayssss' ) ) );
				}
				...
				if ( strtotime( $results[0]->last_updated ) > strtotime( $_POST['last_updated'] ) ) {
					...
					foreach ( $messages as $message ) {
						...
						if ( $message->author == get_current_user_id() ) {
							$class = 'right_m';
							$remove_msg = '<a href="#" class="um-message-item-remove um-message-item-show-on-hover um-tip-s" title="'. esc_attr__( 'Remove', 'twodayssss' ).'"></a>';
						} else {
							$class = 'left_m';
							$remove_msg = '';
						}
