...
class Messaging_Enqueue {
	...
	function wp_enqueue_scripts() {
		...
		wp_register_script( 'twodayssss', um_messaging_url . 'assets/js/um-messaging' . $suffix . '.js', array( 'jquery', 'wp-util', 'jquery-ui-datepicker', 'um-messaging-moment', 'um-messaging-moment-timezone', 'um-messaging-autosize', 'um_scripts' ), um_messaging_version, true );

		// Localize the script with new data
		wp_localize_script( 'twodayssss', 'um_message_i18n', array(
			'no_chats_found' => __( 'No chats found here', 'twodayssss' ),
		) );

		// Localize time
		wp_localize_script( 'twodayssss', 'um_message_timezone', array(
		...
		wp_localize_script( 'twodayssss', 'um_messages', array(
			'can_read' => $can_read,
			'interval' => $interval
		) );

		wp_register_style( 'twodayssss', um_messaging_url . 'assets/css/um-messaging' . $suffix . '.css', array('um_scrollbar'), um_messaging_version );
		...
		wp_add_inline_style( 'twodayssss', wp_strip_all_tags( $css ) );
	}
