<?php foreach ( $comments as $comment ) {
	...
	<div class="um-activity-commentwrap" data-comment_id="<?php echo esc_attr( $comment->comment_ID ); ?>">
		...
		<div class="um-activity-commentl" id="commentid-<?php echo esc_attr( $comment->comment_ID ); ?>">
			...
			<div class="um-activity-comment-hidden hidden-<?php echo esc_attr( $user_hidden ); ?>"><?php _e('Comment hidden. <a href="#" class="um-link">Show this comment</a>','twodayssss'); ?></div>
				...
				<div class="um-activity-comment-meta">
					<?php if ( is_user_logged_in() ) { ?>
						...
						<?php if ( UM()->Activity_API()->api()->user_liked_comment( $comment->comment_ID ) ) { ?>
							<span><a href="#" class="um-link um-activity-comment-like active" data-like_text="<?php _e('Like','twodayssss'); ?>" data-unlike_text="<?php _e('Unlike','twodayssss'); ?>"><?php _e('Unlike','twodayssss'); ?></a></span>
						<?php } else { ?>
							<span><a href="#" class="um-link um-activity-comment-like" data-like_text="<?php _e('Like','twodayssss'); ?>" data-unlike_text="<?php _e('Unlike','twodayssss'); ?>"><?php _e('Like','twodayssss'); ?></a></span>
						<?php } ?>
						...
						<?php if ( UM()->Activity_API()->api()->can_comment() ) { ?><span><a href="javascript:void(0);" class="um-link um-activity-comment-reply" data-commentid="<?php echo $comment->comment_ID; ?>"><?php _e('Reply','twodayssss'); ?></a></span><?php } ?>
		...
		<div class="um-activity-comment-child">
			...
			if ( $child_all > count( $child ) ) {
				...
				if ( $calc > 1 ) {
					$text = sprintf(__('load <span class="um-activity-more-count">%s</span> more replies','twodayssss'), $calc );
				} else if ( $calc == 1 ) {
					$text = sprintf(__('load <span class="um-activity-more-count">%s</span> more reply','twodayssss'), $calc );
				}
				echo '<a href="javascript:void(0);" class="um-activity-ccommentload" data-load_replies="'. __('load more replies','twodayssss').'" data-load_comments="'.__('load more comments','twodayssss') . '" data-loaded="'. count( $child ) . '"><i class="um-icon-forward"></i><span>' . $text . '</span></a>';
