	...
	class myCRED_Query_Export {
		...
		protected function set_column_headers( $headers = array() ) {
			...
			if ( empty( $headers ) || ! is_array( $headers ) )
				$headers = array( 'ref' => __( 'Reference', 'twodayssss' ), 'ref_id' => __( 'Reference ID', 'twodayssss' ), 'user_id' => __( 'User', 'twodayssss' ), 'creds' => __( 'Amount', 'twodayssss' ), 'ctype' => __( 'Point Type', 'twodayssss' ), 'time' => __( 'Date', 'twodayssss' ), 'entry' => __( 'Entry', 'twodayssss' ), 'data' => __( 'Data', 'twodayssss' ) );
			...
			if ( ! empty( $this->headers ) ) {
				foreach ( $this->headers as $header_id ) {
					...
					switch ( $header_id ) {
						...
						case 'user_id' :
						case __( 'User', 'twodayssss' ) :

							$user         = get_userdata( $entry->user_id );
							$display_name = '<span>' . __( 'User Missing', 'twodayssss' ) . ' (ID: ' . $entry->user_id . ')</span>';
...
if ( ! function_exists( 'mycred_get_export_formats' ) ) :
	function mycred_get_export_formats() {
		...
		return apply_filters( 'mycred_export_formats', array(
			'raw'       => __( 'Export log entries raw', 'twodayssss' ),
			'formatted' => __( 'Export log entries formatted', 'twodayssss' )
		) );
...
if ( ! function_exists( 'mycred_get_log_exports' ) ) :
	function mycred_get_log_exports() {
		...
		$defaults = array(
			'all'      => array(
				'label'    => __( 'All Log Entries', 'twodayssss' ),
				'my_label' => NULL,
				'class'    => 'btn btn-primary button button-primary'
			),
			'search'   => array(
				'label'    => __( 'Search Results', 'twodayssss' ),
				'my_label' => NULL,
				'class'    => 'btn btn-primary button button-secondary'
			),
			'user'     => array(
				'label'    => __( 'Users Log Entries', 'twodayssss' ),
				'my_label' => __( 'Export History', 'twodayssss' ),
				'class'    => 'btn btn-primary button button-secondary'
			)
		);
