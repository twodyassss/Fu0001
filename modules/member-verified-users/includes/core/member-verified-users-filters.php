...
function um_verified_extend_bulk_actions( $actions ){
	$actions['um_verify_accounts'] = array( 'label' => __( 'Mark accounts as verified', 'twodayssss' ) );
	$actions['um_unverify_accounts'] = array( 'label' => __( 'Mark accounts as unverified', 'twodayssss' ) );
	return $actions;
}
...
function um_verified_users_settings( $settings ) {
    ...
    $settings['extensions']['sections'][$key] = array(
        'title'     => __( 'Verified Users', 'twodayssss' ),
        'fields'    => array(
            array(
                'id'        => 'verified_redirect',
                'type'      => 'text',
                'label'     => __( 'Content Lock Redirect', 'twodayssss' ),
                'tooltip'   => __('Unverified users who access verified areas will be redirected to that URL.','twodayssss'),
                'size'      => 'medium',
            )
        )
    );
...
function um_verified_user_row_actions( $actions, $user_id ) {
	...
	if ( $status == 'unverified' ) {
		$actions['verify'] = "<a class='' href='" . $verify_url . "'>" . __( 'Verify','twodayssss') . "</a>";
	} elseif ( $status == 'pending' ) {
		$actions['verify'] = "<a class='' href='" . $verify_url . "'>" . __( 'Approve verification request','twodayssss') . "</a>";
		$actions['unverify'] = "<a class='' href='" . $unverify_url . "'>" . __( 'Reject verification','twodayssss') . "</a>";
	} elseif ( $status == 'verified' ) {
		$actions['unverify'] = "<a class='' href='" . $unverify_url . "'>" . __( 'Unverify','twodayssss') . "</a>";
	}
...
function um_verified_activity_action( $actions ) {
	$actions['verified-account'] = __('Account Verifications','twodayssss');
	return $actions;
}
...
function um_verified_admin_views_users( $views ) {
	...
	$views['needs-verification'] = '<a href="'.admin_url('users.php').'?status=needs-verification" ' . $current . '>'. __('Request Verification','twodayssss') . ' <span class="count">(' . UM()->Verified_Users_API()->api()->verified_requests_count() . ')</span></a>';
	return $views;
}
...
function um_verified_add_notification_type( $array ) {
	$array['account_verified'] = array(
		'title'         => __( 'User account is verified','twodayssss' ),
		'template'      => 'Congratulations! Your account is now verified.',
		'account_desc'  => __( 'When my account gets verified', 'twodayssss' ),
	);
	return $array;
}
...
function um_verified_profile_completeness_roles_metabox_fields( $fields, $role ) {
	...
	$fields[] = array(
		...
		'type'		    => 'select',
		'label'		    => __( 'Verify user account', 'twodayssss' ),
		'tooltip'	=> __( 'Verify the user\'s account when he/she completes his/her profile.', 'twodayssss' ),
...
function um_verification_field( $content, $user ) {
	...
	if ( current_user_can( 'edit_users' ) && current_user_can( 'edit_user', $user->ID ) ) {
		...
		<table class="form-table">
			...
			<tr>
				<th>
					<label for="um_set_verification"><?php _e( 'Account Verification', 'twodayssss' ); ?></label>
				</th>
				<td>
					<select name="um_set_verification" id="um_set_verification">
						<option value='0' <?php selected( 0, $is_verified ); ?>><?php _e('Unverified Account','twodayssss'); ?></option>
						<option value='1'  <?php selected( 1, $is_verified ); ?>><?php _e('Verified Account','twodayssss'); ?></option>
					</select>
				</td>
			</tr>
			</tbody>
...
function um_admin_access_settings_fields( $fields, $data ) {
	$fields[] = array(
		...
		'type'		    => 'checkbox',
		'label'    		=> __( 'Lock content to verified accounts only?', 'twodayssss' ),
...
function um_verified_add_role_metabox( $roles_metaboxes ) {
	...
	$roles_metaboxes[] = array(
		...
		'title'    => __( 'Verified Accounts', 'twodayssss' ),
...
function um_verified_sort_user_option( $options ) {
	$options['verified_first'] = __( 'Verified accounts first', 'twodayssss' );
