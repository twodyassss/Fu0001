	...
	class myCRED_Network_Module {
		...
		public function site_column_content( $column_name, $blog_id ) {
			...
			if ( $column_name == MYCRED_SLUG ) {
				...
				if ( mycred_is_site_blocked( $blog_id ) ) {
					...
					echo '<span class="dashicons dashicons-warning"></span><div class="row-actions"><span class="info" style="color: #666">' . __( 'Blocked', 'twodayssss' ) . '</span></div>';
					...
					if ( ! $this->settings['master'] ) {
						...
						if ( get_blog_option( $blog_id, 'mycred_setup_completed', false ) !== false )
							echo '<span class="dashicons dashicons-yes" style="color: green;"></span><div class="row-actions"><span class="info" style="color: #666">' . __( 'Installed', 'twodayssss' ) . '</span></div>';
						else
							echo '<span class="dashicons dashicons-minus"></span><div class="row-actions"><span class="info" style="color: #666">' . __( 'Not Installed', 'twodayssss' ) . '</span></div>';

					}
					else {

						echo '<span class="dashicons dashicons-yes"' . ( $blog_id == 1 ? ' style="color: green;"' : '' ) . '></span><div class="row-actions"><span class="info" style="color: #666">' . ( $blog_id == 1 ? __( 'Master Template', 'twodayssss' ) : __( 'Enabled', 'twodayssss' ) ) . '</span></div>';
		...
		public function add_menu() {
			...
			$pages[] = add_submenu_page(
				MYCRED_SLUG . '-network',
				__( 'Network Settings', 'twodayssss' ),
				__( 'Network Settings', 'twodayssss' ),
				'manage_network_options',
				MYCRED_SLUG . '-network',
		...
		public function admin_page_header() {
			...
			wp_localize_script( 'mycred-accordion', 'twodayssss', array( 'active' => 0 ) );
...
h4.ui-accordion-header.ui-state-active:before { content: "<?php _e( 'click to close', 'twodayssss' ); ?>"; }
h4.ui-accordion-header:before { content: "<?php _e( 'click to open', 'twodayssss' ); ?>"; }
</style>
<?php
...
<div class="wrap mycred-metabox" id="myCRED-wrap">
	<h1><?php printf( __( '%s Network', 'twodayssss' ), $name ); ?><?php if ( MYCRED_DEFAULT_LABEL === 'twodayssss' ) : ?> <a href="" class="page-title-action" target="_blank"><?php _e( 'Documentation', 'twodayssss' ); ?></a><?php endif; ?></h1>
<?php

			if ( wp_is_large_network() ) {

?>
	<p><?php _e( 'I am sorry but your network is too big to use these features.', 'twodayssss' ); ?></p>
<?php
				...
				if ( $setup === false )
					echo '<div class="error"><p>' . sprintf( __( 'Note! %s has not yet been setup.', 'twodayssss' ), $name ) . '</p></div>';

				// Settings Updated
				if ( isset( $_GET['settings-updated'] ) )
					echo '<div class="updated"><p>' . __( 'Settings Updated', 'twodayssss' ) . '</p></div>';
...
?>
	<form method="post" action="<?php echo admin_url( 'options.php' ); ?>" class="form" name="mycred-core-settings-form" novalidate>
		...
		<div class="list-items expandable-li" id="accordion">
			...
			<h4><span class="dashicons dashicons-admin-settings static"></span><label><?php _e( 'Settings', 'twodayssss' ); ?></label></h4>
			<div class="body" style="display: none;">

				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
						<h3><?php _e( 'Master Template', 'twodayssss' ); ?></h3>
						<p><a href="" target="_blank"><?php _e( 'Documentation', 'twodayssss' ); ?></a></p>
						<div class="row">
							<div class="col-xs-6">
								<div class="form-group">
									<label for="mycred-network-overwrite-enabled"><input type="radio" name="mycred_network[master]" id="mycred-network-overwrite-enabled" <?php checked( (int) $this->settings['master'], 1 ); ?> value="1" /> <?php _e( 'Enabled', 'twodayssss' ); ?></label>
								</div>
							</div>
							<div class="col-xs-6">
								<div class="form-group">
									<label for="mycred-network-overwrite-disabled"><input type="radio" name="mycred_network[master]" id="mycred-network-overwrite-disabled" <?php checked( (int) $this->settings['master'], 0 ); ?> value="0" /> <?php _e( 'Disabled', 'twodayssss' ); ?></label>
								</div>
							</div>
						</div>
					</div>
					<div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
						<h3><?php _e( 'Central Logging', 'twodayssss' ); ?></h3>
						<p><a href="" target="_blank"><?php _e( 'Documentation', 'twodayssss' ); ?></a></p>
						<div class="row">
							<div class="col-xs-6">
								<div class="form-group">
									<label for="mycred-network-overwrite-log-enabled"><input type="radio" name="mycred_network[central]" id="mycred-network-overwrite-log-enabled" <?php checked( (int) $this->settings['central'], 1 ); ?> value="1" /> <?php _e( 'Enabled', 'twodayssss' ); ?></label>
								</div>
							</div>
							<div class="col-xs-6">
								<div class="form-group">
									<label for="mycred-network-overwrite-log-disabled"><input type="radio" name="mycred_network[central]" id="mycred-network-overwrite-log-disabled" <?php checked( (int) $this->settings['central'], 0 ); ?> value="0" /> <?php _e( 'Disabled', 'twodayssss' ); ?></label>
								</div>
							</div>
						</div>
					</div>
				...
				<h3><?php _e( 'Site Block', 'twodayssss' ); ?></h3>
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="form-group">
							<label for="mycred-network-block"><?php _e( 'Blog IDs', 'twodayssss' ); ?></label>
							...
							<p><span class="description"><?php printf( __( 'Comma separated list of blog ids where %s is to be disabled.', 'twodayssss' ), $name ); ?></span></p>
						</div>
					</div>
				</div>
		...
		<?php submit_button( __( 'Save Network Settings', 'twodayssss' ), 'primary large', 'submit' ); ?>
