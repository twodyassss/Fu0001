...
if ( ! class_exists( 'myCRED_Query_Log' ) ) :
	class myCRED_Query_Log {
		...
		public function table_headers() {
			...
			$columns = array(
				'username' => __( 'User', 'twodayssss' ),
				'time'     => __( 'Date', 'twodayssss' ),
				'creds'    => $this->core->plural(),
				'entry'    => __( 'Entry', 'twodayssss' )
			);
			...
			if ( array_key_exists( 'creds', $headers ) && count( $this->types ) > 1 )
				$headers['creds'] = __( 'Amount', 'twodayssss' );
		...
		public function get_no_entries() {
			...
			return __( 'No log entries found', 'twodayssss' );
		...
		public function bulk_actions() {
			...
			$bulk_actions = apply_filters( 'mycred_log_bulk_actions', array(
				'-1'            => __( 'Bulk Actions', 'twodayssss' ),
				'export-raw'    => __( 'Export Raw', 'twodayssss' ),
				'export-format' => __( 'Export Formatted', 'twodayssss' ),
				'delete'        => __( 'Delete', 'twodayssss' )
			), $this );
...
?>
	...
	<input type="submit" class="button action" id="doaction" value="<?php _e( 'Apply', 'twodayssss' ); ?>" />
</div>
<?php
		...
		public function filter_options( $is_profile = false ) {
			...
			if ( ! empty( $this->references ) ) {
				...
				echo '<select name="ref" id="myCRED-reference-filter"><option value="">' . __( 'Show all references', 'twodayssss' ) . '</option>';
			...
			if ( $this->core->user_is_point_editor() && ! $is_profile && $this->num_rows > 0 ) {
				...
				echo '<input type="text" class="form-control" name="user" id="myCRED-user-filter" size="22" placeholder="' . __( 'User ID, Username, Email or Nicename', 'twodayssss' ) . '" value="' . ( ( isset( $_GET['user'] ) ) ? esc_attr( $_GET['user'] ) : '' ) . '" /> ';
				$show = true;
			...
			if ( $this->num_rows > 0 ) {
				...
				echo '<select name="order" id="myCRED-order-filter"><option value="">' . __( 'Show in order', 'twodayssss' ) . '</option>';
				foreach ( array( 'ASC' => __( 'Ascending', 'twodayssss' ), 'DESC' => __( 'Descending', 'twodayssss' ) ) as $value => $label ) {
			...
			if ( $show === true )
				echo '<input type="submit" class="btn btn-default button button-secondary" value="' . __( 'Filter', 'twodayssss' ) . '" />';
...
?>
<h2 class="screen-reader-text sr-only"><?php _e( 'Log entries navigation', 'twodayssss' ); ?></h2>
...
<?php
		...
		public function pagination( $location = 'top', $id = '' ) {
			...
			if ( $this->have_entries() )
				$output = '<span class="displaying-num">' . sprintf( _n( '1 entry', '%d entries', $this->num_rows, 'twodayssss' ), $this->num_rows ) . '</span>';
	...
	<thead>
		<tr>';
			...
			foreach ( $this->headers as $col_id => $col_title ) {
				...
				if ( $col_id == 'cb' )
					$output .= '<td id="cb" class="manage-column column-cb check-column"><label class="screen-reader-text" for="cb-select-all-1">' . __( 'Select all', 'twodayssss' ) . '</label><input type="checkbox" id="cb-select-all-1" /></td>';
	...
	<tfoot>
		<tr>';
			...
			foreach ( $this->headers as $col_id => $col_title ) {
				...
				if ( $col_id == 'cb' )
					$output .= '<td class="manage-column column-cb check-column"><label class="screen-reader-text" for="cb-select-all-2">' . __( 'Select all', 'twodayssss' ) . '</label><input type="checkbox" id="cb-select-all-2" /></td>';
			...
			foreach ( $this->headers as $column_id => $column_name ) {
				...
				switch ( $column_id ) {
					...
					case 'cb' :
						...
						$entry_data .= '<th scope="row" class="check-column"><label class="screen-reader-text" for="mycred-log-entry' . $log_entry->id . '">' . __( 'Select entry', 'twodayssss' ) . '</label><input type="checkbox" name="entry[]" id="mycred-log-entry' . $log_entry->id . '" value="' . $log_entry->id . '" /></th>';
					...
					case 'username' :
						...
						$display_name = '<span>' . __( 'User Missing', 'twodayssss' ) . ' (ID: ' . $log_entry->user_id . ')</span>';
						...
						if ( $this->is_admin )
							$content .= '<button type="button" class="toggle-row"><span class="screen-reader-text">' . __( 'Show more details', 'twodayssss' ) . '</span></button>';
					...
					case 'time' :
						...
						if ( $this->is_admin )
							$content .= '<div class="row-actions"><span class="view"><a href="' . add_query_arg( array( 'page' => $_REQUEST['page'], 'time' => $this->get_time_for_filter( $log_entry->time ) ), admin_url( 'admin.php' ) ) . '">' . __( 'Filter by Date', 'twodayssss' ) . '</a></span></div>';
		...
		public function get_row_actions( $entry, $user ) {
			...
			$filter_label = __( 'Filter by User', 'twodayssss' );
			if ( $user === false )
				$filter_label = __( 'Filter by ID', 'twodayssss' );
			...
			$actions['edit']   = '<a href="javascript:void(0);" class="mycred-open-log-entry-editor" data-id="' . $entry->id . '" data-ref="' . $entry->ref . '">' . __( 'Edit', 'twodayssss' ) . '</a>';
			$actions['delete'] = '<a href="javascript:void(0);" class="mycred-delete-row" data-id="' . $entry->id . '">' . __( 'Delete', 'twodayssss' ) . '</a>';
...
?>
	<p><span class="description"><?php _e( 'Log entries are exported to a CSV file and depending on the number of entries selected, the process may take a few seconds.', 'twodayssss' ); ?></span></p>
<?php
				...
				echo '<p>' . __( 'No export options available.', 'twodayssss' ) . '</p>';
...
?>
<p class="search-box">
	<label class="screen-reader-text"><?php _e( 'Search Log', 'twodayssss' ); ?>:</label>
	<input type="search" name="s" value="<?php echo esc_attr( $serarch_string ); ?>" placeholder="<?php _e( 'search log entries', 'twodayssss' ); ?>" />
	<input type="submit" id="search-submit" class="button button-medium button-secondary" value="<?php _e( 'Search Log', 'twodayssss' ); ?>" />
</p>
<?php
		...
		public function filter_dates( $url = '' ) {
			...
			$date_sorting = apply_filters( 'mycred_sort_by_time', array(
				''          => __( 'All', 'twodayssss' ),
				'today'     => __( 'Today', 'twodayssss' ),
				'yesterday' => __( 'Yesterday', 'twodayssss' ),
				'thisweek'  => __( 'This Week', 'twodayssss' ),
				'thismonth' => __( 'This Month', 'twodayssss' )
			) );
...
if ( ! function_exists( 'mycred_get_all_references' ) ) :
	function mycred_get_all_references() {
		...
		// Hooks
		$hooks = array(
			'registration'        => __( 'Website Registration', 'twodayssss' ),
			'site_visit'          => __( 'Website Visit', 'twodayssss' ),
			'view_content'        => __( 'Viewing Content (Member)', 'twodayssss' ),
			'view_content_author' => __( 'Viewing Content (Author)', 'twodayssss' ),
			'logging_in'          => __( 'Logging in', 'twodayssss' ),
			'publishing_content'  => __( 'Publishing Content', 'twodayssss' ),
			'approved_comment'    => __( 'Approved Comment', 'twodayssss' ),
			'unapproved_comment'  => __( 'Unapproved Comment', 'twodayssss' ),
			'spam_comment'        => __( 'SPAM Comment', 'twodayssss' ),
			'deleted_comment'     => __( 'Deleted Comment', 'twodayssss' ),
			'link_click'          => __( 'Link Click', 'twodayssss' ),
			'watching_video'      => __( 'Watching Video', 'twodayssss' ),
			'visitor_referral'    => __( 'Visitor Referral', 'twodayssss' ),
			'signup_referral'     => __( 'Signup Referral', 'twodayssss' )
		);

		if ( class_exists( 'BuddyPress' ) ) {
			$hooks['new_profile_update']     = __( 'New Profile Update', 'twodayssss' );
			$hooks['deleted_profile_update'] = __( 'Profile Update Removal', 'twodayssss' );
			$hooks['upload_avatar']          = __( 'Avatar Upload', 'twodayssss' );
			$hooks['upload_cover']           = __( 'Profile Cover Upload', 'twodayssss' );
			$hooks['new_friendship']         = __( 'New Friendship', 'twodayssss' );
			$hooks['ended_friendship']       = __( 'Ended Friendship', 'twodayssss' );
			$hooks['new_comment']            = __( 'New Profile Comment', 'twodayssss' );
			$hooks['comment_deletion']       = __( 'Profile Comment Deletion', 'twodayssss' );
			$hooks['fave_activity']          = __( 'Add Activity to Favorites', 'twodayssss' );
			$hooks['unfave_activity']        = __( 'Remove Activity from Favorites', 'twodayssss' );
			$hooks['new_message']            = __( 'New Message', 'twodayssss' );
			$hooks['sending_gift']           = __( 'Sending Gift', 'twodayssss' );
			$hooks['creation_of_new_group']  = __( 'New Group', 'twodayssss' );
			$hooks['deletion_of_group']      = __( 'Deleted Group', 'twodayssss' );
			$hooks['new_group_forum_topic']  = __( 'New Group Forum Topic', 'twodayssss' );
			$hooks['edit_group_forum_topic'] = __( 'Edit Group Forum Topic', 'twodayssss' );
			$hooks['new_group_forum_post']   = __( 'New Group Forum Post', 'twodayssss' );
			$hooks['edit_group_forum_post']  = __( 'Edit Group Forum Post', 'twodayssss' );
			$hooks['joining_group']          = __( 'Joining Group', 'twodayssss' );
			$hooks['leaving_group']          = __( 'Leaving Group', 'twodayssss' );
			$hooks['upload_group_avatar']    = __( 'New Group Avatar', 'twodayssss' );
			$hooks['upload_group_cover']     = __( 'New Group Cover', 'twodayssss' );
			$hooks['new_group_comment']      = __( 'New Group Comment', 'twodayssss' );
		}

		if ( function_exists( 'bpa_init' ) || function_exists( 'bpgpls_init' ) ) {
			$hooks['photo_upload'] = __( 'Photo Upload', 'twodayssss' );
			$hooks['video_upload'] = __( 'Video Upload', 'twodayssss' );
			$hooks['music_upload'] = __( 'Music Upload', 'twodayssss' );
		}

		if ( function_exists( 'bp_links_setup_root_component' ) ) {
			$hooks['new_link']    = __( 'New Link', 'twodayssss' );
			$hooks['link_voting'] = __( 'Link Voting', 'twodayssss' );
			$hooks['update_link'] = __( 'Link Update', 'twodayssss' );
		}

		if ( class_exists( 'bbPress' ) ) {
			$hooks['new_forum'] = __( 'New Forum (bbPress)', 'twodayssss' );
			$hooks['new_forum_topic'] = __( 'New Forum Topic (bbPress)', 'twodayssss' );
			$hooks['topic_favorited'] = __( 'Favorited Topic (bbPress)', 'twodayssss' );
			$hooks['new_forum_reply'] = __( 'New Topic Reply (bbPress)', 'twodayssss' );
		}

		if ( function_exists( 'wpcf7' ) )
			$hooks['contact_form_submission'] = __( 'Form Submission (Contact Form 7)', 'twodayssss' );

		if ( class_exists( 'GFForms' ) )
			$hooks['gravity_form_submission'] = __( 'Form Submission (Gravity Form)', 'twodayssss' );

		if ( defined( 'SFTOPICS' ) ) {
			$hooks['new_forum_topic'] = __( 'New Forum Topic (SimplePress)', 'twodayssss' );
			$hooks['new_topic_post']  = __( 'New Forum Post (SimplePress)', 'twodayssss' );
		}

		if ( class_exists( 'Affiliate_WP' ) ) {
			$hooks['affiliate_signup']          = __( 'Affiliate Signup (AffiliateWP)', 'twodayssss' );
			$hooks['affiliate_visit_referral']  = __( 'Referred Visit (AffiliateWP)', 'twodayssss' );
			$hooks['affiliate_referral']        = __( 'Affiliate Referral (AffiliateWP)', 'twodayssss' );
			$hooks['affiliate_referral_refund'] = __( 'Referral Refund (AffiliateWP)', 'twodayssss' );
		}

		if ( defined( 'WP_POSTRATINGS_VERSION' ) ) {
			$hooks['post_rating']        = __( 'Adding a Rating', 'twodayssss' );
			$hooks['post_rating_author'] = __( 'Receiving a Rating', 'twodayssss' );
		}

		if ( function_exists( 'vote_poll' ) )
			$hooks['poll_voting'] = __( 'Poll Voting', 'twodayssss' );

		if ( function_exists( 'invite_anyone_init' ) ) {
			$hooks['sending_an_invite']   = __( 'Sending an Invite', 'twodayssss' );
			$hooks['accepting_an_invite'] = __( 'Accepting an Invite', 'twodayssss' );
		}

		// Addons
		$addons = array();
		if ( class_exists( 'myCRED_Banking_Module' ) ) {
			$addons['interest']  = __( 'Compound Interest', 'twodayssss' );
			$addons['recurring'] = __( 'Recurring Payout', 'twodayssss' );
		}

		if ( class_exists( 'myCRED_Badge_Module' ) )
			$hooks['badge_reward'] = __( 'Badge Reward', 'twodayssss' );

		if ( class_exists( 'myCRED_buyCRED_Module' ) ) {
			$addons['buy_creds_with_paypal_standard'] = __( 'buyCRED Purchase (PayPal Standard)', 'twodayssss' );
			$addons['buy_creds_with_skrill']          = __( 'buyCRED Purchase (Skrill)', 'twodayssss' );
			$addons['buy_creds_with_zombaio']         = __( 'buyCRED Purchase (Zombaio)', 'twodayssss' );
			$addons['buy_creds_with_netbilling']      = __( 'buyCRED Purchase (NETBilling)', 'twodayssss' );
			$addons['buy_creds_with_bitpay']          = __( 'buyCRED Purchase (BitPay)', 'twodayssss' );
			$addons['buy_creds_with_bank']            = __( 'buyCRED Purchase (Bank Transfer)', 'twodayssss' );
			$addons = apply_filters( 'mycred_buycred_refs', $addons );
		}

		if ( class_exists( 'myCRED_Coupons_Module' ) )
			$addons['coupon'] = __( 'Coupon Usage', 'twodayssss' );

		if ( defined( 'myCRED_GATE' ) ) {
			if ( class_exists( 'WooCommerce' ) ) {
				$addons['woocommerce_payment'] = __( 'Store Purchase (WooCommerce)', 'twodayssss' );
				$addons['reward']              = __( 'Store Reward (WooCommerce)', 'twodayssss' );
				$addons['product_review']      = __( 'Product Review (WooCommerce)', 'twodayssss' );
			}
			if ( class_exists( 'MarketPress' ) ) {
				$addons['marketpress_payment'] = __( 'Store Purchase (MarketPress)', 'twodayssss' );
				$addons['marketpress_reward']  = __( 'Store Reward (MarketPress)', 'twodayssss' );
			}
			if ( class_exists( 'wpsc_merchant' ) )
				$addons['wpecom_payment']      = __( 'Store Purchase (WP E-Commerce)', 'twodayssss' );

			$addons = apply_filters( 'mycred_gateway_refs', $addons );
		}

		if ( defined( 'EVENT_ESPRESSO_VERSION' ) ) {
			$addons['event_payment']   = __( 'Event Payment (Event Espresso)', 'twodayssss' );
			$addons['event_sale']      = __( 'Event Sale (Event Espresso)', 'twodayssss' );
		}

		if ( defined( 'EM_VERSION' ) ) {
			$addons['ticket_purchase'] = __( 'Event Payment (Events Manager)', 'twodayssss' );
			$addons['ticket_sale']     = __( 'Event Sale (Events Manager)', 'twodayssss' );
		}

		if ( class_exists( 'myCRED_Sell_Content_Module' ) ) {
			$addons['buy_content']  = __( 'Content Purchase', 'twodayssss' );
			$addons['sell_content'] = __( 'Content Sale', 'twodayssss' );
		}

		if ( class_exists( 'myCRED_Transfer_Module' ) )
			$addons['transfer'] = __( 'Transfer', 'twodayssss' );

		$references = array_merge( $hooks, $addons );

		$references['manual'] = __( 'Manual Adjustment by Admin', 'twodayssss' );
