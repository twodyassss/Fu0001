...
function um_followers_follow_button_in_directory( $user_id, $args ) {
	...
	if ( UM()->options()->get( 'followers_show_stats' ) && $can_view ) { ?>
		<div class="um-members-follow-stats">
			<div><?php echo UM()->Followers_API()->api()->count_followers( $user_id ); ?> <?php _e( 'followers', 'twodayssss' ); ?></div>
			<div><?php echo UM()->Followers_API()->api()->count_following( $user_id ); ?> <?php _e( 'following', 'twodayssss' ); ?></div>
		</div>
	<?php }
