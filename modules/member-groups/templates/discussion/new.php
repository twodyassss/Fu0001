...
if( $total_pending_reviews > 0 && is_user_logged_in() && 'require-moderation' == $group_moderation && ! $hide_wall_post  ){
	...
	if( UM()->Groups()->api()->can_manage_group( $group_id, $user_id, $privacy ) ){
		echo sprintf( _n("%s post requires approval","%s posts require approval",$total_pending_reviews,'twodayssss'),$total_pending_reviews );
	}else{
		echo sprintf( _n("You have %s post requires admin approval","You have %s posts require admin approval",$total_pending_reviews,'twodayssss'),$total_pending_reviews );
	}
	echo "</a>";
	echo "</div>";
}
...
<div class="um-groups-widget um-groups-new-post" <?php if( $hide_wall_post ){ echo "style='display:none;'"; } ?> >
	...
	<div class="um-groups-head">
		<?php echo ( um_profile_id() == get_current_user_id() ) ? __('Write Post','twodayssss') : sprintf(__('Post on %s\'s wall','twodayssss'), um_user('display_name') ); ?>
	</div>
	
	<div class="um-groups-body">

		<div class="um-groups-textarea">
			<textarea data-photoph="<?php _e('Say something about this photo','twodayssss'); ?>" data-ph="<?php _e('Write something...','twodayssss'); ?>" placeholder="<?php _e('Write something...','twodayssss'); ?>" class="um-groups-textarea-elem" name="_post_content" id="_post_content"></textarea>
		</div>
	...
	<div class="um-groups-foot">
		...
		<div class="um-groups-left um-groups-insert">
			...
			<a href="#" class="um-groups-insert-photo um-tip-s" data-timestamp="<?php echo $timestamp;?>" data-nonce="<?php echo $nonce;?>" title="<?php _e('Add photo','twodayssss'); ?>" data-allowed="gif,png,jpeg,jpg" data-size-err="<?php _e('Image is too large','twodayssss'); ?>" data-ext-err="<?php _e('Please upload a valid image','twodayssss'); ?>"><i class="um-faicon-camera"></i></a>
			<?php } ?>
		...
		<div class="um-groups-right">
			...
			<a href="#" class="um-button um-groups-post um-disabled"><?php _e('Post','twodayssss'); ?></a>
