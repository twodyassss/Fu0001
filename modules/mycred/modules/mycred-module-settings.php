	...
	class myCRED_Settings_Module extends myCRED_Module {
		...
		public function __construct( $type = MYCRED_DEFAULT_TYPE_KEY ) {
			...
			parent::__construct( 'myCRED_Settings_Module', array(
				...
				'labels'      => array(
					'menu'        => __( 'Settings', 'twodayssss' ),
					'page_title'  => __( 'Settings', 'twodayssss' ),
					'page_header' => __( 'Settings', 'twodayssss' )
				),
		...
		public function action_export_balances() {
			...
			if ( empty( $query ) )
				wp_send_json_error( __( 'No users found to export', 'twodayssss' ) );
		...
		public function action_update_log_cred_format() {
			...
			if ( ! isset( $_POST['decimals'] ) || $_POST['decimals'] == '' || $_POST['decimals'] > 20 )
				wp_send_json_error( __( 'Invalid decimal value.', 'twodayssss' ) );
			...
			wp_send_json_success( array(
				...
				'label' => __( 'Log Updated', 'twodayssss' )
			) );
		...
		public function settings_header() {
			...
			if ( isset( $_REQUEST['open-tab'] ) && array_key_exists( $_REQUEST['open-tab'], $this->accordion_tabs ) )
				wp_localize_script( 'mycred-accordion', 'twodayssss', array( 'active' => $this->accordion_tabs[ $_REQUEST['open-tab'] ] ) );
			...
			wp_localize_script(
				...
				array(
					...
					'working'       => esc_attr__( 'Processing...', 'twodayssss' ),
					'confirm_log'   => esc_attr__( 'Warning! All entries in your log will be permanently removed! This can not be undone!', 'twodayssss' ),
					'confirm_clean' => esc_attr__( 'All log entries belonging to deleted users will be permanently deleted! This can not be undone!', 'twodayssss' ),
					'confirm_reset' => esc_attr__( 'Warning! All user balances will be set to zero! This can not be undone!', 'twodayssss' ),
					'done'          => esc_attr__( 'Done!', 'twodayssss' ),
					'export_close'  => esc_attr__( 'Close', 'twodayssss' ),
					'export_title'  => $mycred->template_tags_general( esc_attr__( 'Export %singular% Balances', 'twodayssss' ) ),
					'decimals'      => esc_attr__( 'In order to adjust the number of decimal places you want to use we must update your log. It is highly recommended that you backup your current log before continuing!', 'twodayssss' )
				)
			);
...
<div class="wrap mycred-metabox" id="myCRED-wrap">
	<h1><?php _e( 'Settings', 'twodayssss' ); if ( MYCRED_DEFAULT_LABEL === 'twodayssss' ) : ?> <a href="" target="_blank" class="page-title-action"><?php _e( 'Documentation', 'twodayssss' ); ?></a><?php endif; ?></h1>

	<?php $this->update_notice(); ?>

	<?php if ( MYCRED_DEFAULT_LABEL === 'twodayssss' ) : ?>
	<p id="mycred-thank-you-text"><?php printf( __( 'Thank you for using %s. If you have a moment, please leave a %s.', 'twodayssss' ), mycred_label(), sprintf( '<a href="https://wordpress.org/support/plugin/mycred/reviews/?rate=5#new-post" target="_blank">%s</a>', __( 'review', 'twodayssss' ) ) ); ?><span id="mycred-social-media"><?php echo implode( ' ', $social ); ?></span></p>
	<?php endif; ?>
	...
	<form method="post" action="options.php" class="form" name="mycred-core-settings-form" novalidate>
		...
		<div class="list-items expandable-li" id="accordion">
			<h4><span class="dashicons dashicons-admin-settings static"></span><label><?php _e( 'Core Settings', 'twodayssss' ); ?></label></h4>
			<div class="body" style="display:none;">

				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<h3><?php _e( 'Labels', 'twodayssss' ); ?></h3>
						<div class="row">
							<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( array( 'name' => 'singular' ) ); ?>"><?php _e( 'Singular', 'twodayssss' ); ?></label>
									<input type="text" name="<?php echo $this->field_name( array( 'name' => 'singular' ) ); ?>" id="<?php echo $this->field_id( array( 'name' => 'singular' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'twodayssss' ); ?>" value="<?php echo esc_attr( $this->core->name['singular'] ); ?>" />
								</div>
							</div>
							<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( array( 'name' => 'plural' ) ); ?>"><?php _e( 'Plural', 'twodayssss' ); ?></label>
									<input type="text" name="<?php echo $this->field_name( array( 'name' => 'plural' ) ); ?>" id="<?php echo $this->field_id( array( 'name' => 'plural' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'twodayssss' ); ?>" value="<?php echo esc_attr( $this->core->name['plural'] ); ?>" />
								</div>
							</div>
						</div>
						<p><span class="description"><?php _e( 'These labels are used throughout the admin area and when presenting points to your users.', 'twodayssss' ); ?></span></p>
					</div>
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<h3><?php _e( 'Format', 'twodayssss' ); ?></h3>
						<div class="row">
							<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( 'before' ); ?>"><?php _e( 'Prefix', 'twodayssss' ); ?></label>
							...
							<div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( array( 'format' => 'separators' ) ); ?>-thousand"><?php _e( 'Separators', 'twodayssss' ); ?></label>
							...
							<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for=""><?php _e( 'Decimals', 'twodayssss' ); ?></label>
									<?php $this->adjust_decimal_places(); ?>
								</div>
							</div>
							<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( 'after' ); ?>"><?php _e( 'Suffix', 'twodayssss' ); ?></label>
						...
						<p><span class="description"><?php _e( 'Set decimals to zero if you prefer to use whole numbers.', 'twodayssss' ); ?></span></p>
						<?php if ( $this->is_main_type ) : ?>
						<p><strong><?php _e( 'Tip', 'twodayssss' ); ?>:</strong> <?php _e( 'As this is your main point type, the value you select here will be the largest number of decimals your installation will support.', 'twodayssss' ); ?></span></p>
						<?php endif; ?>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<h3><?php _e( 'Security', 'twodayssss' ); ?></h3>
						<div class="row">
							<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( array( 'caps' => 'creds' ) ); ?>"><?php _e( 'Point Editors', 'twodayssss' ); ?></label>
									<input type="text" name="<?php echo $this->field_name( array( 'caps' => 'creds' ) ); ?>" id="<?php echo $this->field_id( array( 'caps' => 'creds' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'twodayssss' ); ?>" value="<?php echo esc_attr( $this->core->caps['creds'] ); ?>" />
									<p><span class="description"><?php _e( 'The capability of users who can edit balances.', 'twodayssss' ); ?></span></p>
								</div>
							</div>
							<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( array( 'caps' => 'plugin' ) ); ?>"><?php _e( 'Point Administrators', 'twodayssss' ); ?></label>
									<input type="text" name="<?php echo $this->field_name( array( 'caps' => 'plugin' ) ); ?>" id="<?php echo $this->field_id( array( 'caps' => 'plugin' ) ); ?>" class="form-control" placeholder="<?php _e( 'Required', 'twodayssss' ); ?>" value="<?php echo esc_attr( $this->core->caps['plugin'] ); ?>" />
									<p><span class="description"><?php _e( 'The capability of users who can edit settings.', 'twodayssss' ); ?></span></p>
								</div>
							</div>
							<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12">
								<div class="form-group">
									<?php if ( ! isset( $this->core->max ) ) $this->core->max(); ?>
									<label for="<?php echo $this->field_id( 'max' ); ?>"><?php _e( 'Max. Amount', 'twodayssss' ); ?></label>
									...
									<p><span class="description"><?php _e( 'The maximum amount allowed to be paid out in a single instance.', 'twodayssss' ); ?></span></p>
								</div>
							</div>
							<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
								<div class="form-group">
									<label for="<?php echo $this->field_id( array( 'exclude' => 'list' ) ); ?>"><?php _e( 'Exclude by User ID', 'twodayssss' ); ?></label>
									<input type="text" name="<?php echo $this->field_name( array( 'exclude' => 'list' ) ); ?>" id="<?php echo $this->field_id( array( 'exclude' => 'list' ) ); ?>" placeholder="<?php _e( 'Optional', 'twodayssss' ); ?>" class="form-control" value="<?php echo esc_attr( $this->core->exclude['list'] ); ?>" />
									<p><span class="description"><?php _e( 'Comma separated list of user IDs to exclude from using this point type.', 'twodayssss' ); ?></span></p>
								</div>
								<div class="form-group">
									<div class="checkbox">
										<label for="<?php echo $this->field_id( array( 'exclude' => 'cred_editors' ) ); ?>"><input type="checkbox" name="<?php echo $this->field_name( array( 'exclude' => 'cred_editors' ) ); ?>" id="<?php echo $this->field_id( array( 'exclude' => 'cred_editors' ) ); ?>"<?php checked( $this->core->exclude['cred_editors'], 1 ); ?> value="1" /> <?php _e( 'Exclude point editors', 'twodayssss' ); ?></label>
									</div>
									<div class="checkbox">
										<label for="<?php echo $this->field_id( array( 'exclude' => 'plugin_editors' ) ); ?>"><input type="checkbox" name="<?php echo $this->field_name( array( 'exclude' => 'plugin_editors' ) ); ?>" id="<?php echo $this->field_id( array( 'exclude' => 'plugin_editors' ) ); ?>"<?php checked( $this->core->exclude['plugin_editors'], 1 ); ?> value="1" /> <?php _e( 'Exclude point administrators', 'twodayssss' ); ?></label>
									</div>
								</div>
							</div>
						</div>
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<h3><?php _e( 'Other Settings', 'twodayssss' ); ?></h3>
						<div class="form-group">
							<label for="<?php echo $this->field_id( 'delete_user' ); ?>"><input type="checkbox" name="<?php echo $this->field_name( 'delete_user' ); ?>" id="<?php echo $this->field_id( 'delete_user' ); ?>" <?php checked( $delete_user, 1 ); ?> value="1" /> <?php _e( 'Delete log entries when user is deleted.', 'twodayssss' ); ?></label>
						</div>
					</div>
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
...
?>
			<h4><span class="dashicons dashicons-dashboard static"></span><label><?php _e( 'Management', 'twodayssss' ); ?></label></h4>
			<div class="body" style="display:none;">
				...
				<div class="row">
					...
					<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Entries', 'twodayssss' ); ?></label>
							<h1><?php echo $total_rows; ?></h1>
						</div>
					</div>
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Actions', 'twodayssss' ); ?></label>
							<div>
								<?php if ( ( ! mycred_centralize_log() ) || ( mycred_centralize_log() && $GLOBALS['blog_id'] == 1 ) ) : ?>
								<button type="button" id="mycred-manage-action-empty-log" data-type="<?php echo $this->mycred_type; ?>" class="button button-large large <?php if ( $total_rows == 0 ) echo '"disabled="disabled'; else echo 'button-primary'; ?>"><?php _e( 'Empty Log', 'twodayssss' ); ?></button>
								<?php endif; ?>
							</div>
						</div>
					</div>
				...
				<div class="row">
					<div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Balance Meta Key', 'twodayssss' ); ?></label>
							<h1><?php echo $this->core->cred_id; ?></h1>
						</div>
					</div>
					<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Users', 'twodayssss' ); ?></label>
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Actions', 'twodayssss' ); ?></label>
							<div>
								<button type="button" id="mycred-manage-action-reset-accounts" data-type="<?php echo $this->mycred_type; ?>" class="button button-large large <?php if ( $reset_block ) echo '" disabled="disabled'; else echo 'button-primary'; ?>"><?php _e( 'Set all to zero', 'twodayssss' ); ?></button> 
								<button type="button" id="mycred-export-users-points" data-type="<?php echo $this->mycred_type; ?>" class="button button-large large"><?php _e( 'Export Balances', 'twodayssss' ); ?></button>
							</div>
						</div>
					</div>
				</div>
...
?>
			<h4><span class="dashicons dashicons-star-filled static"></span><label><?php _e( 'Point Types', 'twodayssss' ); ?></label></h4>
			<div class="body" style="display:none;">
<?php
...
?>
				<div class="row">
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<label>&nbsp;</label>
							<label><input type="checkbox" disabled="disabled" class="disabled" value="<?php echo esc_attr( $type ); ?>" /> <?php _e( 'Delete', 'twodayssss' ); ?></label>
						</div>
					</div>
				</div>
<?php
...
?>
				<div class="row">
					...
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<label>&nbsp;</label>
							<label for="mycred-point-type-<?php echo esc_attr( $type ); ?>"><input type="checkbox" name="mycred_pref_core[delete_types][]" id="mycred-point-type-<?php echo esc_attr( $type ); ?>" value="<?php echo esc_attr( $type ); ?>" /> <?php _e( 'Delete', 'twodayssss' ); ?></label>
						</div>
					</div>
				</div>
<?php
...
?>
				<h3><?php _e( 'Add New Type', 'twodayssss' ); ?></h3>
				<div class="row">
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<label for="mycred-new-ctype-key-value"><?php _e( 'Meta Key', 'twodayssss' ); ?></label>
							<input type="text" id="mycred-new-ctype-key-value" name="mycred_pref_core[types][new][key]" placeholder="<?php _e( 'Required', 'twodayssss' ); ?>" value="" class="form-control" />
						</div>
					</div>
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
						<div class="form-group">
							<label for="mycred-new-ctype-key-label"><?php _e( 'Label', 'twodayssss' ); ?></label>
							<input type="text" id="mycred-new-ctype-key-label" name="mycred_pref_core[types][new][label]" placeholder="<?php _e( 'Required', 'twodayssss' ); ?>" value="" class="form-control" />
						</div>
					</div>
					<div class="col-lg-4 col-md-4 col-sm-12 col-xs-12"></div>
				</div>
				<p id="mycred-ctype-warning"><strong><?php _e( 'The meta key must be lowercase and only contain letters or underscores. All other characters will be deleted!', 'twodayssss' ); ?></strong></p>
			</div>
<?php
...
?>
		...
		<?php submit_button( __( 'Update Settings', 'twodayssss' ), 'primary large', 'submit' ); ?>
		...
		<div class="mycred-container">
			...
			<div class="form mycred-metabox">
				<div class="row">
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Identify users by', 'twodayssss' ); ?></label>
							<select id="mycred-export-identify-by" class="form-control">
<?php

			// Identify users by...
			$identify = apply_filters( 'mycred_export_by', array(
				'ID'    => __( 'User ID', 'twodayssss' ),
				'email' => __( 'User Email', 'twodayssss' ),
				'login' => __( 'User Login', 'twodayssss' )
			) );
...
?>
							...
							<span class="description"><?php _e( 'Use ID if you intend to use this export as a backup of your current site while Email is recommended if you want to export to a different site.', 'twodayssss' ); ?></span>
						</div>
					</div>
					<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
						<div class="form-group">
							<label><?php _e( 'Import Log Entry', 'twodayssss' ); ?></label>
							<input type="text" id="mycred-export-log-template" value="" class="regular-text form-control" />
							<span class="description"><?php echo sprintf( __( 'Optional log entry to use if you intend to import this file in a different %s installation.', 'twodayssss' ), mycred_label() ); ?></span>
						</div>
					</div>
				</div>	
				...
				<div class="row last">
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-right">
						<input type="button" id="mycred-run-exporter" value="<?php _e( 'Export', 'twodayssss' ); ?>" data-type="<?php echo $this->mycred_type; ?>" class="button button-large button-primary" />
					</div>
				</div>
			</div>
