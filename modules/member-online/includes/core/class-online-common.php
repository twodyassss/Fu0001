...
class Online_Common {
	...
	function admin_settings( $settings ) {
		...
		$settings['extensions']['sections'][ $key ] = array(
			'title'     => __( '在线状态', 'twodayssss' ),
			'fields'    => array(
				array(
					'id'    => 'online_show_stats',
					'type'  => 'checkbox',
					'label' => __( '在会员目录显示在线状态', 'twodayssss' ),
				),
			)
		);
	...
	function wp_enqueue_scripts() {
		...
		wp_register_script( 'twodayssss', um_online_url . 'assets/js/um-online' . $suffix . '.js', array( 'jquery' ), um_online_version, true );
		wp_register_style( 'twodayssss', um_online_url . 'assets/css/um-online' . $suffix . '.css', array( 'um_styles' ), um_online_version );
	}
	...
	function um_online_show_user_status( $args ) {
		...
		twodays_get_template( 'online-marker.php', um_online_plugin, $args, true );
	...
	function um_online_add_fields( $fields ) {
		...
		$fields['_hide_online_status'] = array(
			'title'         => __( '显示我的在线状态?', 'twodayssss' ),
			'metakey'       => '_hide_online_status',
			'type'          => 'radio',
			'label'         => __( '显示我的在线状态?', 'twodayssss' ),
			'help'          => __( '你想让别人看到你在线吗?', 'twodayssss' ),
			'required'      => 0,
			'public'        => 1,
			'editable'      => 1,
			'default'       => 'yes',
			'options'       => array( 'yes' => __( '是的', 'twodayssss' ), 'no' => __( '不是', 'twodayssss' ) ),
			'account_only'  => true,
		);
	...
	function um_online_show_status( $value, $data ) {
		...
		twodays_get_template( 'online-text.php', um_online_plugin, $args, true );
