...
class Groups_Discussion {
	...
	function __construct() {
		...
		$this->global_actions['new-group'] = __('New Group','twodayssss');
		$this->global_actions['new-gform'] = __('New Gravity From','twodayssss');
		$this->global_actions['new-gform-submission'] = __('New Gravity From Answer','twodayssss');
	...
	function shorten_string( $string ){
		...
		if (count( $array ) <= $wordsreturned) {
			...
			$retval = implode( " ", $array ) . " <span class='um-groups-seemore'>(<a href='' class='um-link'>" . __( 'See more', 'twodayssss' ) . "</a>)</span>" . " <span class='um-groups-hiddentext'>" . implode( " ", $res ) . "</span>";
		}
	...
	function human_time_diff( $from, $to = '' ){
		...
		if ($diff < 60) {
			...
			if ($days <= 1)
				$days = 1;
			if ($days == 1) {
				$since = sprintf( __( 'Yesterday at %s', 'twodayssss' ), date_i18n( __( 'g:ia', 'twodayssss' ), $from ) );
			} else {
				$since = sprintf( __( '%s at %s', 'twodayssss' ), date_i18n( __( 'F d', 'twodayssss' ), $from ), date_i18n( __( 'g:ia', 'twodayssss' ), $from ) );
			}
			...
			$since = sprintf( __( '%s at %s', 'twodayssss' ), date_i18n( __( 'F d, Y', 'twodayssss' ), $from ), date_i18n( __( 'g:ia', 'twodayssss' ), $from ) );
	...
	function get_faces( $post_id, $num = 10 ) {
		...
		return '<a href="#" data-post_id="' . $post_id . '" class="um-activity-show-likes um-tip-s" title="' . __( 'People who like this', 'twodayssss' ) . '" data-post_id="' . $post_id . '">' . $res . '</a>';
	}
	...
	function ajax_load_wall() {
		...
		if( in_array( $slug, array('private','hidden') ) && ! UM()->Groups()->api()->has_joined_group( get_current_user_id() , absint( $_POST['group_id'] )  ) ){
			return wp_send_json( array('restricted' => __("You do not have the necessary permission for the specified Group to perform this action.",'twodayssss' ) ) );
		}
