...
class Followers_Main_API {
	...
	function rest_get_following( $args ) {
		...
		if ( ! $id ) {
			$error['error'] = __( 'You must provide a user ID', 'twodayssss' );
			return $error;
		}

		$results = UM()->Followers_API()->api()->following( $id );
		if ( !$results ) {
			$error['error'] = __( 'No users were found', 'twodayssss' );
			return $error;
		}
	...
	function follow_button( $user_id1, $user_id2 ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			$res = '<a href="' . $redirect . '" class="um-login-to-follow-btn um-button um-alt">'. __('Follow','twodayssss'). '</a>';
			return $res;
		}
		...
		if ( $this->can_follow( $user_id1, $user_id2 ) ) {
			...
			if ( ! $this->followed( $user_id1, $user_id2 ) ) {
				$res = '<a href="#" class="um-follow-btn um-button um-alt" data-user_id1="'.$user_id1.'" data-user_id2="'.$user_id2.'">'. __('Follow','twodayssss'). '</a>';
			} else {
				$res = '<a href="#" class="um-unfollow-btn um-button" data-user_id1="'.$user_id1.'" data-user_id2="'.$user_id2.'" data-following="'.__('Following','twodayssss').'"  data-unfollow="'.__('Unfollow','twodayssss').'">'. __('Following','twodayssss'). '</a>';
			}
