...
class Activity_Admin {
	...
	***/
	function prevent_backend_new() {
		...
		if( $current_screen->id == 'um_activity'){
			wp_die( __('This can be done from the frontend only.','twodayssss') );
		}
	}
	...
	***/
	function um_extend_admin_menu() {
		...
		add_submenu_page( $this->slug, __('Social Activity','twodayssss'), sprintf(__('Social Activity %s','twodayssss'), $count ), 'manage_options', 'edit.php?post_type=um_activity', '' );
		
		add_submenu_page( $this->slug, __('Hashtags','twodayssss'), __('Hashtags','twodayssss'), 'manage_options', 'edit-tags.php?taxonomy=um_hashtag', '' );
    ...
    function manage_edit_um_activity_columns($columns) {
        ...
        $columns['a_author'] = __('Author','twodayssss');
        $columns['a_content'] = __('Content','twodayssss');
        $columns['a_hashtags'] = __('Hashtags','twodayssss');
        $columns['a_likes'] = __('Likes','twodayssss');
        $columns['a_comments'] = __('Comments','twodayssss');
        $columns['a_action'] = __('Action','twodayssss');
    ...
    function manage_um_activity_posts_custom_column($column_name, $id) {
        switch ($column_name) {
            ...
            case 'a_content':
                ...
                echo '<a href="' . get_edit_post_link( $id ) . '" class="um-admin-tipsy-s" title="'.__('Edit','twodayssss').'"><strong>'. UM()->Activity_API()->api()->get_post_time( $id ) . '</strong></a> - <a href="' . UM()->Activity_API()->api()->get_permalink( $id ) . '" target="_blank">' .__('Permalink','twodayssss').'</a>';
                ...
                if ( UM()->Activity_API()->api()->reported( $id ) ) {
                    ...
                    echo '<div class="um-admin-activity-reported">' . sprintf(__('This post is flagged by community. <a href="%s">Clear report</a>','twodayssss'), $clear_report ) . '</div>';
                }
