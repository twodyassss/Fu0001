...
function um_groups_directory_search_form( $args ) {
	...
	echo '<div class="um-groups-directory-header">';
		...
		echo '<input type="text" name="groups_search" placeholder="'.__('Search groups...', 'twodayssss' ).'" value="'.esc_attr( $search ).'"/>';
		...
		echo '<input type="submit" class="" value="'.__('Search', 'twodayssss' ).'"/> ';
		echo '<a href="'. get_the_permalink() .'" class="primary-button">'.__('Clear', 'twodayssss' ).'</a>';
	...
	if ( um_groups('total_groups') > 0 ) {
		...
		foreach( um_groups('groups') as $group ): 
			...
			if( true == $args['show_actions'] ){
				...
				echo sprintf( _n( '<span>%s</span> member', '<span>%s</span> members', $count, 'twodayssss' ), number_format_i18n( $count ) );
				echo '</li>';

				echo '<li class="last-active">';
				echo '<span>';
				echo __('active, ','twodayssss').human_time_diff( UM()->Groups()->api()->get_group_last_activity( $group->ID, true ) ).__(' ago','twodayssss');
				echo '</span>';
				echo '</li>';
			...
			echo sprintf( __('%s Group', 'twodayssss' ), um_groups_get_privacy_title( $group->ID ) );
			echo '</li>';
...
function um_groups_directory_tabs( $args ) {
	...
	echo '<li class="all '.( 'all' == $filter || empty( $filter ) ? 'active': '' ).' "><a href="'.um_get_core_page('groups').'">'.sprintf( __('All Groups <span>%s</span>','twodayssss'), um_groups_get_all_groups_count() ).'</a></li>';

	if( is_user_logged_in() ){
		echo '<li class="own '.( 'own' == $filter ? 'active': '' ).'"><a href="'.um_get_core_page('groups').'?filter=own">'.sprintf( __('My Groups <span>%s</span>', 'twodayssss'), um_groups_get_own_groups_count() ).'</a></li>';
		echo '<li class="create"><a href="'.um_get_core_page('create_group').'">'.__('Create a Group', 'twodayssss').'</a></li>';
	}
...
function um_groups_own_directory_tabs( $args ) {
	...
	echo '<div class="um-groups-found-own-posts">'.sprintf( __('All Groups <span>%s</span>','twodayssss'), UM()->Groups()->api()->get_own_groups_count() ).'</div>';
}
