...
class UM_Groups_Admin_Init {
	...
	public function um_groups_add_meta_boxes( $post ) {
		add_meta_box( 'um_groups_settings_meta_box', __( 'Settings', 'twodayssss' ), array($this,'um_groups_build_meta_box'), 'um_groups', 'normal', 'high' );
		add_meta_box( 'um_groups_invites_meta_box', __( 'Invites settings', 'twodayssss' ), array($this,'um_groups_build_invites_meta_box'), 'um_groups', 'normal', 'high' );
		...
		add_meta_box( 'um_groups_add_members_meta_box', __( 'Add New Members', 'twodayssss' ), array($this,'um_groups_add_members_meta_box'), 'um_groups', 'normal', 'high' );
		add_meta_box( 'um_groups_manage_members_meta_box', __( 'Manage Members', 'twodayssss' ), array($this,'um_groups_manage_members_meta_box'), 'um_groups', 'normal', 'high' );
	}
	...
	public function save_metabox_groups( $post_id, $post, $update ) {
		...
		if ( empty( $_POST['post_title'] ) ) {
			$_POST['post_title'] = __( 'Group #', 'twodayssss' ) . $post_id;
			...
			<div id="major-publishing-actions">
				...
				<div id="delete-action">
					<?php if ( current_user_can( "delete_post", $post->ID ) ) {
						if ( ! EMPTY_TRASH_DAYS ) {
							$delete_text = __( 'Delete Group Permanently', 'twodayssss' );
						} else {
							$delete_text = __( 'Delete Group', 'twodayssss' );
						} ?>
				...
				<div id="publishing-action">
					...
					<?php if ( ! in_array( $post->post_status, array('publish', 'future', 'private') ) || 0 == $post->ID ) {
						if ( $can_publish ) { ?>
							<input name="original_publish" type="hidden" id="original_publish" value="<?php esc_attr_e( 'Add Tab', 'twodayssss' ) ?>" />
							<?php submit_button( __( 'Add Group' ), 'primary button-large', 'publish', false, array( 'accesskey' => 'p' ) ); ?>
						<?php }
					} else { ?>
						<input name="original_publish" type="hidden" id="original_publish" value="<?php esc_attr_e('Update Group', 'twodayssss' ); ?>" />
						<input name="save" type="submit" class="button button-primary button-large" id="publish" accesskey="p" value="<?php esc_attr_e('Update Group', 'twodayssss'); ?>" />
					<?php } ?>
				</div>
				<div class="clear"></div>
			</div>
