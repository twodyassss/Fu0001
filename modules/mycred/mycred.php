...
if ( ! class_exists( 'myCRED_Core' ) ) :
	final class myCRED_Core {
		...
		private function define_constants() {
			...
			$this->define( 'MYCRED_SLUG',                 'twodayssss' );
			$this->define( 'MYCRED_DEFAULT_LABEL',        'twodayssss' );
			...
			$this->define( 'myCRED_URL', 				  trailingslashit( trailingslashit(get_template_directory_uri().'/modules') . basename( dirname( __FILE__ ) ) ) );
			$this->define( 'myCRED_ROOT_DIR',             trailingslashit(__DIR__), false );
		...
		public function wordpress() {
			...
			//add_action( 'init',    array( $this, 'after_plugin' ), 999 );
		...
		public function load_plugin_textdomain() {
			...
			// Load Translation
			$locale = apply_filters( 'plugin_locale', get_locale(), 'twodayssss' );

			load_textdomain( 'twodayssss', myCRED_LANG_DIR . 'mycred-zh_CN.mo' );
		...
		public function register_assets() {
			...
			// Styles
			wp_register_style( 'mycred-front',          myCRED_URL.'assets/css/mycred-front.css',         array(), $this->version, 'all' );
			wp_register_style( 'mycred-admin',          myCRED_URL. 'assets/css/mycred-admin.css',        array(), $this->version, 'all' );
			wp_register_style( 'mycred-edit-balance',   myCRED_URL. 'assets/css/mycred-edit-balance.css', array(), $this->version, 'all' );
			wp_register_style( 'mycred-edit-log',       myCRED_URL. 'assets/css/mycred-edit-log.css',     array(), $this->version, 'all' );
			wp_register_style( 'mycred-bootstrap-grid', myCRED_URL. 'assets/css/bootstrap-grid.css',      array(), $this->version, 'all' );
			wp_register_style( 'mycred-forms',          myCRED_URL. 'assets/css/mycred-forms.css',        array(), $this->version, 'all' );

			// Scripts
			wp_register_script( 'mycred-send-points',   myCRED_URL. 'assets/js/send.js',                  array( 'jquery' ), $this->version, true );
			wp_register_script( 'mycred-accordion',     myCRED_URL. 'assets/js/mycred-accordion.js',      array( 'jquery', 'jquery-ui-core', 'jquery-ui-accordion' ), $this->version );
			wp_register_script( 'jquery-numerator',     myCRED_URL. 'assets/libs/jquery-numerator.js',    array( 'jquery' ), '0.2.1' );
			wp_register_script( 'mycred-mustache',      myCRED_URL. 'assets/libs/mustache.min.js',        array(), '2.2.1' );
			wp_register_script( 'mycred-widgets',       myCRED_URL. 'assets/js/mycred-admin-widgets.js',  array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-draggable', 'jquery-ui-droppable' ), $this->version );
			wp_register_script( 'mycred-edit-balance',  myCRED_URL. 'assets/js/mycred-edit-balance.js',   array( 'jquery', 'jquery-ui-core', 'jquery-ui-dialog', 'jquery-effects-core', 'jquery-effects-slide', 'jquery-numerator' ), $this->version );
			wp_register_script( 'mycred-edit-log',      myCRED_URL. 'assets/js/mycred-edit-log.js',       array( 'jquery', 'jquery-ui-core', 'jquery-ui-dialog', 'jquery-effects-core', 'jquery-effects-slide', 'common' ), $this->version );
		...
		private function register_importers() {
			...
			register_importer(
				...
				sprintf( __( '%s Log Import', 'twodayssss' ), mycred_label() ),
				__( 'Import log entries via a CSV file.', 'twodayssss' ),
			...
			register_importer(
				...
				sprintf( __( '%s Balance Import', 'twodayssss' ), mycred_label() ),
				__( 'Import balances via a CSV file.', 'twodayssss' ),
			...
			register_importer(
				...
				sprintf( __( '%s CubePoints Import', 'twodayssss' ), mycred_label() ),
				__( 'Import CubePoints log entries and / or balances.', 'twodayssss' ),
		...
		public function enqueue_front_after() {
			...
			if ( $mycred_sending_points === true || apply_filters( 'mycred_enqueue_send_js', false ) === true ) {
				...
				$language = apply_filters( 'mycred_send_language', array(
					...
					'done'    => esc_attr__( 'Sent', 'twodayssss' ),
					'error'   => esc_attr__( 'Error - Try Again', 'twodayssss' )
				) );
		...
		public function adjust_admin_menu() {
			...
			$pages[]   = add_dashboard_page(
				sprintf( __( 'About %s', 'twodayssss' ), $name ),
				sprintf( __( 'About %s', 'twodayssss' ), $name ),
				'moderate_comments',
				MYCRED_SLUG . '-about',
				'mycred_about_page'
		...
		public function adjust_toolbar( $wp_admin_bar ) {
			...
			$main_label = __( 'Balance', 'twodayssss' );
			...
			foreach ( $usable_types as $type_id ) {
				...
				if ( $history_url != '' && apply_filters( 'mycred_admin_show_history_' . $type_id, true ) === true )
					$wp_admin_bar->add_menu( array(
						...
						'title'  => sprintf( '%s %s', $point_type->plural(), __( 'History', 'twodayssss' ) ),
						'href'   => $history_url
					) );
		...
		public function plugin_links( $actions, $plugin_file, $plugin_data, $context ) {
			...
			// Link to Setup
			if ( ! mycred_is_installed() )
				$actions['_setup'] = '<a href="' . admin_url( 'plugins.php?page=' . MYCRED_SLUG . '-setup' ) . '">' . __( 'Setup', 'twodayssss' ) . '</a>';
			else
				$actions['_settings'] = '<a href="' . admin_url( 'admin.php?page=' . MYCRED_SLUG . '-settings' ) . '" >' . __( 'Settings', 'twodayssss' ) . '</a>';
