...
<form id="um-user-photos-form-edit-album" class="um-user-photos-modal-form" action="<?= admin_url('admin-ajax.php?action=update_um_user_photos_album'); ?>" method="post" enctype="multipart/form-data"  data-max_size_error="<?= __(' is too large. File should be less than ','twodayssss'); ?>" data-max_size="<?= wp_max_upload_size(); ?>">
	<div class="um-galley-form-response"></div>
	<div>
		<input
				type="text"
				placeholder="<?= __('Album title','twodayssss'); ?>"
				name="album_title"
	...
	<div>
		<div class="text-center">
			<h1 class="album-poster-holder" style="background-image:url(<?= $bg_image; ?>);">
				<label class="album-poster-label">
					...
					<span><?= __('Album cover','twodayssss'); ?></span>
					<input
				...
				for($i=0;$i<count($photos);$i++):
					...
					<div class="um-user-photos-photo" id="album-photo-<?= $photos[$i]; ?>">
						...
						<a class="photo-delete-link um-tip-n"
						   ...
						   original-title="<?= __('Delete photo','twodayssss'); ?>"
						   data-confirmation="<?= __('Sure to delete photo?','twodayssss'); ?>"
		...
		<p class="text-center">
			<label
				...
				<i class="um-icon-plus"></i> <?= __('Add photos','twodayssss'); ?>
	...
	<div class="um-user-photos-modal-footer text-right">
		...
		><?= __('Update','twodayssss'); ?></button>
                
		<a href="javascript:void(0);" class="um-modal-btn alt um-user-photos-modal-close-link"><?= __('Cancel','twodayssss'); ?></a>
	</div>
