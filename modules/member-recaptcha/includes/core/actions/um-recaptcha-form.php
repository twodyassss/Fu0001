...
function um_recaptcha_add_captcha( $args ) {
	...
	switch( $version ) {
		case 'v3':
			...
			$t_args = compact( 'args' );
			twodays_get_template( 'captcha_v3.php', um_recaptcha_plugin, $t_args, true );
		...
		default:
			...
			twodays_get_template( 'captcha.php', um_recaptcha_plugin, $t_args, true );
...
function um_recaptcha_validate( $args ) {
	...
	$error_codes = array(
		'missing-input-secret'   => __( 'The secret parameter is missing.', 'twodayssss' ),
		'invalid-input-secret'   => __( 'The secret parameter is invalid or malformed.', 'twodayssss' ),
		'missing-input-response' => __( 'Please confirm you are not a robot', 'twodayssss' ),
		'invalid-input-response' => __( 'The response parameter is invalid or malformed.', 'twodayssss' ),
		'bad-request'            => __( 'The request is invalid or malformed.', 'twodayssss' ),
		'timeout-or-duplicate'   => __( 'The response is no longer valid: either is too old or has been used previously.', 'twodayssss' ),
	);
