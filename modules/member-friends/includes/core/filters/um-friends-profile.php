...
function um_friends_profile_privacy_options( $options ) {
	$options = array_merge( $options, array(
		'friends' => __( 'Friends only', 'twodayssss' ),
	) );
...
function um_friends_messaging_privacy_options( $options ) {
	$options['friends'] = __( 'Friends', 'twodayssss' );
	return $options;
}
...
function um_friends_user_add_tab( $tabs ) {
	...
	$myfriends = ( um_is_myprofile() ) ? __( 'My Friends', 'twodayssss' ) : sprintf( __( '%s\'s friends', 'twodayssss' ), $username );
	...
	if ( um_is_myprofile() ) {
		...
		$tabs['friends']['subnav']['friendreqs'] = __( 'Friend Requests', 'twodayssss') . '<span class="'. $class . '">' . $new_reqs . '</span>';
		$tabs['friends']['subnav']['sentreqs'] = __( 'Friend Requests Sent', 'twodayssss') . '<span>' . UM()->Friends_API()->api()->count_friend_requests_sent( $user_id ) . '</span>';
	}
...
function um_friends_can_view_main( $can_view, $user_id ) {
	if ( ! is_user_logged_in() || get_current_user_id() != $user_id ) {
		...
		if ( ( $is_private_case || $is_private_case_old ) && ! current_user_can( 'manage_options' ) ) { //Enable admin to be able to view
			$can_view = __( 'You must be a friend of this user to view their profile', 'twodayssss' );
		}
	}
...
function um_friends_private_filter_hook( $default, $option, $user_id ) {
	...
	if ( $option == 'friends' || $option == __( 'Friends only', 'twodayssss' ) ) {
